#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v11.
...
USER_AGENT = "perp-inefficiency-scanner/11.0"
...
class Config:
    ...
    active_lag_outcome_max_age_minutes: float = float(os.getenv("ACTIVE_LAG_OUTCOME_MAX_AGE_MINUTES", "30"))
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        "V11 keeps the verified convergence logic and additionally emits informational ACTIVE-LAG and ACTIVE-MM-EXCURSION signals; no orders are ever placed. "
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if event_type == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V11 counter: {symbol} ACTIVE-LAG detections <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
    ...
    if event_type == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V11 counter: {symbol} ACTIVE-MM excursion events <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
...
def lag_verified_profile_rows(state: dict, symbol: str, side: str, cfg: Config, now: Optional[float] = None) -> List[dict]:
    ...
    for row in _state_rows(state, "lag_verified_profiles", symbol):
        ...
        if fnum(row.get("max_convergence_fraction")) < cfg.lag_min_convergence_fraction:
            continue
        # V11 does not trust legacy verified rows created before invalid-BBO
...
def ensure_v11_state(state: dict) -> bool:
    """One-time safe state migration.

    Legacy V10 verified LAG rows are intentionally not deleted (audit trail), but
    V11 profile readers ignore them because they lack BBO coverage/version fields.
    """
    changed = False
    if int(fnum(state.get("state_schema_version"))) < 11:
        state["state_schema_version"] = 11; changed = True
    for key, default in (
...
def update_active_lag_outcomes(state: dict, candidates: List[Candidate], cfg: Config, now_ts: Optional[float]=None) -> Tuple[List[dict], bool]:
    ...
    for event_id, row in list(bucket.items()):
        ...
        if c and c.samples:
            # Use only samples after the signal timestamp and only valid executable BBO.
            for s in c.samples:
                if s.ts < fnum(row.get("start_ts")):
                    continue
                edge = s.hedge_long_edge_pct if side == "LONG" else s.hedge_short_edge_pct
                if not math.isfinite(edge) or s.external_bid <= 0 or s.external_ask <= 0:
                    continue
                ...
                elapsed = max(0.0, s.ts - fnum(row.get("start_ts")))
                ...
                if row.get("full_ts") is None and edge <= cfg.lag_full_convergence_edge_pct:
                    row["full_ts"] = elapsed
            changed=True
        age = now_ts - fnum(row.get("start_ts"))
        final=None
        if row.get("full_ts") is not None:
            final="FULL"
        elif age >= max_age:
            if row.get("t80") is not None: final="T80_TIMEOUT"
            elif row.get("t50") is not None: final="T50_TIMEOUT"
            else: final="STALE"
        if final:
            row["status"]=final; row["closed_ts"]=now_ts; changed=True
            st=_outcome_bucket(state,"ACTIVE-LAG")
...
def format_outcome_signal(event: dict, state: dict) -> str:
    ...
    if typ=="ACTIVE-LAG-OUTCOME":
        return (f"📊 <b>ACTIVE-LAG OUTCOME — {symbol} @ {label}</b>\n"
                f"Side {event.get('side')} | entry gap {fnum(event.get('entry_gap_pct')):.3f}% | min {fnum(event.get('min_gap_pct')):.3f}% | max {fnum(event.get('max_gap_pct')):.3f}%\n"
...
def scan(
    ...
    snap = fetch_v10_snapshot(cfg, candidates=None, initial=True)
    errors.extend(snap.errors)
    print_reference_health(snap)
    candidates = prefilter_v10_candidates(snap, aster_stats, cfg)
    if not candidates:
        # Even on a quiet run, expire old forward outcomes so they do not stay
        # OPEN forever just because their symbol missed the current prefilter.
        lag_outcomes, lag_changed = update_active_lag_outcomes(state, [], cfg)
        ...
        if cfg.active_outcome_alerts_enabled and active_signal_callback is not None:
            for event in lag_outcomes + mm_outcomes:
                active_signal_callback(event)
        return [], errors, bool(migrated or lag_changed or mm_changed)
    ...
    for c in candidates:
        ...
        if m:
            ranked.append((c, m))
    ranked.sort(key=lambda x: x[1]["score"], reverse=True)
    return ranked, errors, bool(migrated or active_changed or extended_changed or same_run_changed or lag_outcome_changed or mm_outcome_changed)
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    if "MM" in m.get("setup", ""):
        caption = (
            ...
            "V11 MM statistics use actual target-venue trades matched to the time-local robust PERP consensus."
        )
    else:
        caption = (
            f"📈 <b>{candidate.symbol} @ {label} — {m['setup']}</b>\n"
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    if "CONVERGING-LAG" in m.get("setup", ""):
        ...
        if m.get("lag_baseline_ready") and base is not None:
            ...
            base_txt = f"median {base:.3f}% ({m.get('lag_baseline_points',0)} pts, READY; required {req:.3f}% if req is not None else 'n/a')"
    ...
    return (
        ...
        f"State: <b>{m['direction']}</b>\n\n"
        "V11 scans Aster, Hyperliquid and Lighter as targets against a robust multi-venue PERP consensus. Signal only; no orders are sent."
    )
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        if baseline is None:
            ...
            btxt = f"{float(baseline):.3f}% ({bp} pts, READY; required {float(br):.3f}% if br is not None else 'n/a')"
        return (
            ...
            f"BBO capacity target/external: {float(event.get('target_bbo_capacity_usd',0)):,.0f} / {float(event.get('external_bbo_capacity_usd',0)):,.0f} USD\n"
            f"V11 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V11 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def print_forward_outcome_stats(state: dict) -> None:
    ...
    print(
        f" - ACTIVE-LAG: started={int(fnum(lag.get('total_started')))} "
        f"t50={int(fnum(lag.get('t50')))} t80={int(fnum(lag.get('t80')))} "
        f"full={int(fnum(lag.get('full')))} stale={int(fnum(lag.get('stale')))}"
    )
    print(
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V11 multi-target MM/LAG + robust consensus + ACTIVE signals + baseline/sanity)"
    )
...
def main() -> int:
    ...
    def active_signal_callback(event: dict) -> None:
        is_outcome = str(event.get("type", "")).endswith("OUTCOME")
