#!/usr/bin/env python3
"""Aster cross-venue algo scanner v7.
...
4) Spot prices are collected separately as diagnostics only, so normal perp/spot basis cannot create a LAG signal.
...
USER_AGENT = "aster-algo-scanner/7.0"
...
class Candidate:
    ...
    def metrics(self, cfg: "Config") -> dict:
        ...
        if mature_mm and lag_confirmed:
            setup = "MATURE-MM+LAG"
            score = max(95.0, lag_score, mm_score)
        elif mature_mm:
            setup = "MATURE-MM"
            score = max(95.0, mm_score)
        elif mm_confirmed and lag_confirmed:
        ...
        level = "CONFIRMED" if (setup.startswith("CONFIRMED") or setup.startswith("MATURE-MM")) and score >= cfg.confirmed_score else "NONE"
...
class Config:
    ...
    # Stage 3: extended verification for the strongest initial MM candidates.
...
def extended_mm_observation(candidates: List[Candidate], cfg: Config, errors: List[str]) -> None:
    """Continuously observe only the strongest *initial* CONFIRMED-MM names.

    The purpose is regime-duration measurement. We keep appending PERP reference
    snapshots and poll recent Aster trades every few seconds, deduplicating them.
    At the end, all accumulated trades are remapped to the time-local PERP fair
    and excursions are recomputed over the entire Stage 2 + Stage 3 window.
    """
    if not cfg.extended_mm_enabled or cfg.extended_mm_duration_seconds <= 0 or not candidates:
        return

    initial: List[Tuple[float, Candidate, dict]] = []
    for c in candidates:
        m = c.metrics(cfg)
        if m.get("setup") in {"CONFIRMED-MM", "CONFIRMED-BOTH"}:
            # Prefer high MM score, then more events and higher clean rate.
            rank = (
                float(m.get("mm_score", 0.0))
                + min(20.0, float(m.get("excursion_count", 0)) * 1.5)
                + float(m.get("clean_reversion_rate", 0.0)) * 10.0
            )
            initial.append((rank, c, m))

    initial.sort(key=lambda x: x[0], reverse=True)
    selected = [c for _, c, _ in initial[: max(0, cfg.extended_mm_max_candidates)]]
    if not selected:
        return

    print(
        "Extended MM verification: "
        f"{', '.join(c.symbol for c in selected)} for ~{cfg.extended_mm_duration_seconds:.0f}s "
        f"(snapshots every {cfg.extended_mm_interval_seconds:.1f}s, "
        f"trades every {cfg.extended_mm_trades_poll_seconds:.1f}s)"
    )

    raw_by_symbol: Dict[str, Dict[tuple, dict]] = {c.symbol: {} for c in selected}
    ...
    # Seed the trade accumulator immediately so the initial confirmation window
    # is likely still present in /trades before high-volume names rotate it out.
    poll_trades()

    start = time.time()
    end = start + cfg.extended_mm_duration_seconds
    next_snapshot = start
    ...
    while time.time() < end:
        ...
        if now >= next_snapshot:
            ...
            try:
                ...
                errors.append(f"extended MM snapshot {sample_no}: {type(e).__name__}: {e}")
            next_snapshot = time.time() + max(0.25, cfg.extended_mm_interval_seconds)
            did_work = True

        now = time.time()
        if now >= next_trade_poll:
            poll_trades()
            next_trade_poll = time.time() + max(1.0, cfg.extended_mm_trades_poll_seconds)
            did_work = True

        if not did_work:
            sleep_for = min(next_snapshot, next_trade_poll, end) - time.time()
            if sleep_for > 0:
                time.sleep(min(0.25, sleep_for))

    # Final poll captures trades near the end of the extended window.
    poll_trades()
    ...
    for c in selected:
        ...
        print(
            ...
            f"excursions={m.get('excursion_count', 0)}, "
            f"clean={m.get('clean_reversion_rate', 0) * 100:.0f}%, "
            f"rate={m.get('excursion_rate_per_minute', 0):.1f}/min, "
            f"quiet_max={m.get('longest_quiet_seconds', 0):.1f}s, "
            f"setup={m.get('setup')}"
        )
...
def scan(cfg: Config) -> Tuple[List[Tuple[Candidate, dict]], List[str]]:
    ...
    # V7: if the initial ~45s window confirms MM behavior, keep watching only
    # the strongest few names for several more minutes before final ranking/alerts.
    extended_mm_observation(candidates, cfg, errors)
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    mm_regime_block = ""
    if m.get("setup") in {"CONFIRMED-MM", "CONFIRMED-BOTH", "MATURE-MM", "MATURE-MM+LAG"}:
    ...
    return (
        ...
        "V7 CONFIRMED-LAG requires a persistent robust hedgeable PERP-to-PERP edge after an estimated round-trip fee reserve. "
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V7 MM + robust hedgeable LAG + thin-edge rejection + "
