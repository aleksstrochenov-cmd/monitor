#!/usr/bin/env python3
"""Aster cross-venue algo scanner v4.
...
USER_AGENT = "aster-algo-scanner/4.0"
...
class Candidate:
    ...
    def metrics(self, cfg: "Config") -> dict:
        ...
        if lag_side == "SHORT":
            direction = "Persistent executable edge is on SHORT side (Aster bid above fair)"
        elif lag_side == "LONG":
            direction = "Persistent executable edge is on LONG side (Aster ask below fair)"
        elif current_dev > 0:
            direction = "Aster midpoint above fair, but no confirmed executable edge"
        elif current_dev < 0:
            direction = "Aster midpoint below fair, but no confirmed executable edge"
        else:
...
def create_price_chart(candidate: Candidate, m: dict, cfg: Config) -> Optional[Path]:
    ...
    ax.set_title(
        ...
        f"Executable edge now {edge:.3f}% {side} | persistent {persistent_side} hit {persistent_hit:.0f}%"
    )
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    caption = (
        ...
        "For LAG, focus on Aster ask (LONG) or bid (SHORT), not the midpoint. Convergence is meaningful only while the reference remains relatively stable."
    )
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        f"Current midpoint deviation: {m['current_deviation_pct']:+.3f}% (diagnostic only)\n"
        ...
        "CONFIRMED-LAG now requires a persistent executable bid/ask edge, median Aster spread below the LAG cap, "
        "and real Aster trades during the observation window. CONFIRMED-MM requires repeated clean trade reversions. "
...
def self_test() -> None:
    ...
    print("Self-test OK (MM + executable LAG + wide-spread false-positive rejection)")
