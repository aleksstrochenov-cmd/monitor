...
def open_paper_trade_from_signal(state: dict, event: dict, cfg: Config) -> bool:
    ...
    if not bool(event.get("execution_ready")):
        return False
    net = fnum(event.get("execution_fresh_net_edge_pct"), -999.0)
    ...
    trade = {
        ...
        "entry_gap_pct": entry_gap, "entry_estimated_net_edge_pct": net,
        ...
        "historical_median_convergence_fraction": fnum(event.get("median_max_convergence_fraction")),
...
def _attach_execution_probe_to_events(events: List[dict], candidates: List[Candidate], cfg: Config, state: Optional[dict] = None) -> List[dict]:
    ...
    for ev in events:
        ...
        # Fresh economics are authoritative for user-facing signal fields.
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V15 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V15 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V16 persistent paper ledger + exact-route PnL + LAG/MM research)"
    )
