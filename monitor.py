#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v15.

Purpose
-------
Read PUBLIC market data only. No exchange API keys and no trading.

V15 deliberately separates market-behaviour research from trade economics:
...
6) POSITIVE-ECONOMICS is a broad alert layer: it does not require prior
   convergence history, so every scanned side that can plausibly be positive is
   probed and alerted only when fresh net economics are positive.
7) ACTIVE-LAG and ACTIVE-MM-EXCURSION remain strategy/context signals, but are
   suppressed unless the same fresh positive-economics gate passes.
8) Funding is tracked for forward outcomes separately because future funding and
   close-book conditions are path-dependent and cannot be guaranteed at entry.
...
USER_AGENT = "perp-inefficiency-scanner/15.0"
...
def ensure_v11_state(state: dict) -> bool:
    """V14 state migration; legacy closed rows remain as an audit trail."""
    changed = False
    if int(fnum(state.get("state_schema_version"))) < 15:
        state["state_schema_version"] = 15; changed = True
    for key, default in (
...
def format_scan_summary_chunks(ranked: List[Tuple[Candidate, dict]], cfg: Config) -> List[str]:
    ...
    header = (
        f"📋 <b>V15 SCAN SUMMARY</b> — checked {len(lines)} markets\n"
    ...
    for line in lines:
        ...
        if len(current) + len(piece) > max_chars and current != header:
            ...
            part += 1
            current = f"📋 <b>V15 SCAN SUMMARY continued {part}</b>\n"
        current += piece
    if current.strip():
        chunks.append(current.rstrip())
    return chunks
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V15 venue-fee economics + positive-edge alerts + broader LAG/MM observation)"
    )
