#!/usr/bin/env python3
"""Aster cross-venue algo scanner v2.
...
1) Fast cross-market prefilter across Aster perpetuals vs Bitget/MEXC/Bybit spot.
2) A short confirmation window that measures:
   - Aster spread and cross-venue deviation
   - conservative executable edge from Aster bid/ask vs external fair price
   - actual Aster trade excursions away from fair value
   - how often those excursions revert
   - how fast they revert
   - whether the external reference stayed stable while Aster reverted
...
USER_AGENT = "aster-algo-scanner/2.0"
...
class Candidate:
    ...
    def metrics(self, cfg: "Config") -> dict:
        ...
        exec_hit_ratio = sum(x >= cfg.min_executable_edge_pct for x in executable_edges) / len(executable_edges)
        ref_good_ratio = sum(x <= cfg.max_reference_disagreement_pct for x in disagreements) / len(disagreements)

        # Direction persistence helps distinguish a stable lag/mispricing regime from random noise.
        ...
        # Score is a ranking heuristic, not expected PnL.
        spread_score = min(18.0, 9.0 * med_spread / max(cfg.min_aster_spread_pct, 1e-9))
        deviation_score = min(18.0, 9.0 * max_abs_dev / max(cfg.min_deviation_pct, 1e-9))
        executable_score = min(16.0, 8.0 * max_exec_edge / max(cfg.min_executable_edge_pct, 1e-9))
        ref_quality_score = 10.0 * ref_good_ratio
        persistence_score = 8.0 * max(dev_hit_ratio, exec_hit_ratio, directional_hit_ratio)
        activity_score = min(5.0, 2.5 * abs(self.move24h_pct) / max(cfg.min_24h_move_pct, 1e-9))

        if excursion_count:
            excursion_frequency_score = min(10.0, 10.0 * excursion_count / max(cfg.min_excursions, 1))
            reversion_quality_score = 10.0 * clean_reversion_rate
            speed_score = 0.0
            if median_clean_reversion_seconds is not None:
                speed_score = 5.0 * max(
                    0.0,
                    min(1.0, cfg.max_median_reversion_seconds / max(median_clean_reversion_seconds, 1e-9)),
                )
        else:
            excursion_frequency_score = 0.0
        ...
        score = min(
            ...
            + persistence_score
            + activity_score
            + excursion_frequency_score
            + reversion_quality_score
            + speed_score,
        )

        # Setup types intentionally separate two market regimes.
        lag_confirmed = (
            ref_good_ratio >= cfg.min_ref_good_ratio
            and max_exec_edge >= cfg.min_executable_edge_pct
            and exec_hit_ratio >= cfg.min_exec_edge_hit_ratio
            and directional_hit_ratio >= cfg.min_directional_hit_ratio
        ...
        if mm_confirmed and lag_confirmed:
            ...
            setup = "WATCH-WIDE"
        elif max_abs_dev >= cfg.min_deviation_pct:
            setup = "WATCH-LAG"
        else:
            setup = "NONE"

        if setup.startswith("CONFIRMED") and score >= cfg.confirmed_score:
            level = "CONFIRMED"
        elif setup.startswith("WATCH") and score >= cfg.watch_score:
            level = "WATCH"
        elif score >= cfg.watch_score and (med_spread >= cfg.min_aster_spread_pct or max_abs_dev >= cfg.min_deviation_pct):
            level = "WATCH"
        else:
            level = "NONE"

        if current.edge_side == "SHORT":
            direction = "Aster ABOVE fair; conservative executable edge is on SHORT side"
        elif current.edge_side == "LONG":
            direction = "Aster BELOW fair; conservative executable edge is on LONG side"
        elif current_dev > 0:
            direction = "Aster above fair, but no conservative taker edge now"
        elif current_dev < 0:
            direction = "Aster below fair, but no conservative taker edge now"
        else:
            direction = "Aster near fair"
...
class Config:
    ...
    min_executable_edge_pct: float = float(os.getenv("MIN_EXECUTABLE_EDGE_PCT", "0.12"))
    ...
    max_trade_analysis_candidates: int = int(os.getenv("MAX_TRADE_ANALYSIS_CANDIDATES", "10"))
    ...
    # Lag/persistent mispricing confirmation
    min_exec_edge_hit_ratio: float = float(os.getenv("MIN_EXEC_EDGE_HIT_RATIO", "0.35"))
    min_directional_hit_ratio: float = float(os.getenv("MIN_DIRECTIONAL_HIT_RATIO", "0.50"))
    ...
    alert_levels: Tuple[str, ...] = tuple(
        x.strip().upper() for x in os.getenv("ALERT_LEVELS", "WATCH,CONFIRMED").split(",") if x.strip()
    )
...
def build_sample(
    ...
    # Conservative edge: assumes an immediate marketable execution on Aster.
    # If Aster is rich, short edge is Aster bid vs fair. If cheap, long edge is fair vs Aster ask.
...
def should_alert(symbol: str, level: str, setup: str, state: dict, cfg: Config) -> bool:
    ...
    # Let WATCH escalate immediately to a confirmed regime; also allow a materially different setup.
...
def create_price_chart(candidate: Candidate, m: dict, cfg: Config) -> Optional[Path]:
    """Create a Telegram-friendly PNG of the confirmation window.

    The chart intentionally shows executable Aster bid/ask rather than a 1-minute candle:
    - external reference fair price
    - Aster bid
    - Aster ask
    - actual Aster trades that exceeded the excursion threshold

    This makes a seconds-scale convergence/lag regime visible even when exchange web charts
    only expose 1-minute candles.
    """
    ...
    ax.plot(xs, fairs, label="Reference fair", linewidth=2.2)
    ...
    # Mark only actual Aster trades that were materially away from the matched reference.
    excursion_trades = [
    ...
    if excursion_trades:
        ...
        ax.scatter(tx, ty, marker="x", s=38, label="Aster excursion trades", zorder=5)

    side = m.get("current_edge_side", "NONE")
    edge = m.get("current_executable_edge_pct", 0.0)
    dev = m.get("current_deviation_pct", 0.0)
    ax.set_title(
        f"{candidate.symbol} | {m['setup']} | ~{xs[-1]:.0f}s\n"
        f"Current deviation {dev:+.3f}% | executable edge {edge:.3f}% {side}"
    )
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    caption = (
        ...
        f"~{duration:.0f}s: reference fair vs Aster bid/ask\n"
        f"Edge now: <b>{m['current_executable_edge_pct']:.3f}% {m['current_edge_side']}</b> | "
        f"deviation {m['current_deviation_pct']:+.3f}%\n"
        "Read it as convergence only if Aster moves toward the reference while the reference itself stays relatively stable."
    )
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        f"Executable edge now: <b>{m['current_executable_edge_pct']:.3f}% {side_text}</b>\n"
        f"Max executable edge: {m['max_executable_edge_pct']:.3f}% | hit ratio {m['exec_hit_ratio'] * 100:.0f}%\n"
        f"Aster spread: median <b>{m['median_spread_pct']:.3f}%</b> | max {m['max_spread_pct']:.3f}%\n"
        f"Deviation: max {m['max_abs_deviation_pct']:.3f}% | hit ratio {m['deviation_hit_ratio'] * 100:.0f}%\n"
        f"Aster trades analyzed: {m['trades_analyzed']}\n"
        f"Excursions ≥ threshold: <b>{m['excursion_count']}</b> | unresolved {m['unresolved_count']}\n"
        f"Reverted: {m['reverted_count']}/{m['excursion_count']} ({m['reversion_rate'] * 100:.0f}%)\n"
        f"Clean reverted: <b>{m['clean_reverted_count']}/{m['excursion_count']} ({m['clean_reversion_rate'] * 100:.0f}%)</b>\n"
        ...
        "Interpretation: CONFIRMED-MM means repeated Aster trade excursions reverted while the external reference stayed relatively stable; "
        "CONFIRMED-LAG means a persistent executable cross-venue mispricing was observed. Manual inspection only."
    )
...
def self_test() -> None:
    cfg = Config(
        ...
        max_trade_analysis_candidates=10,
        ...
        min_exec_edge_hit_ratio=0.35,
        min_directional_hit_ratio=0.50,
        min_ref_good_ratio=0.80,
        watch_score=55,
        confirmed_score=70,
        alert_levels=("WATCH", "CONFIRMED"),
        cooldown_minutes=60,
    ...
    now = time.time()
    # Fair stays stable; Aster is generally wide and occasionally rich/cheap.
    ...
    for i, dev in enumerate(devs):
        ...
        c.samples.append(MarketSample(now + i, aq, fair, refs, 0.0, dev, short_edge, long_edge))

    trade_devs = [0.02, 0.27, 0.29, 0.04, -0.25, -0.31, -0.02, 0.24, 0.03]
    c.trades = [
        TradePoint(now + i, 10 * (1 + d / 100), 100, 10.0, d)
        for i, d in enumerate(trade_devs)
    ]
    c.excursions = detect_excursions(c.trades, cfg)
    m = c.metrics(cfg)
    assert len(c.excursions) >= 3, c.excursions
    assert m["clean_reversion_rate"] >= 0.70, m
    assert m["setup"] in {"CONFIRMED-MM", "CONFIRMED-BOTH"}, m
    assert m["level"] == "CONFIRMED", m
