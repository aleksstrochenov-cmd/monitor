#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v14.
...
1) Broad multi-target discovery. LAG discovery is independent of 24h coin movement: an executable cross-venue spread is evaluated on its own merits.
   MM discovery keeps its own volatility/liquidity filters.
...
USER_AGENT = "perp-inefficiency-scanner/14.0"
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if event_type == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V14 counter: {symbol} ACTIVE-LAG detections <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
    ...
    if event_type == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V14 counter: {symbol} ACTIVE-MM excursion events <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    if "MM" in m.get("setup", ""):
        caption = (
            ...
            "V14 MM statistics use actual target-venue trades matched to the time-local robust PERP consensus."
        )
    else:
        caption = (
            f"📈 <b>{candidate.symbol} @ {label} — {m['setup']}</b>\n"
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        f"State: <b>{m['direction']}</b>\n\n"
        "V14 scans Aster, Hyperliquid and Lighter against a robust PERP consensus, keeps execution-readiness checks, and follows ACTIVE-LAG outcomes for up to 24h with funding carry. Signal only; no orders are sent."
    )
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V14 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V14 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
# ---------------------------------------------------------------------------
# V14 discovery + per-run gate audit
# ---------------------------------------------------------------------------
...
def _v14_lag_failures(m: dict, cfg: Config) -> List[str]:
    ...
    if spr > cfg.max_lag_median_spread_pct: fails.append(f"spr {spr:.2f}>{cfg.max_lag_median_spread_pct:.2f}")
    ...
    if net < cfg.min_net_hedgeable_edge_pct: fails.append(f"net {net:+.3f}<{cfg.min_net_hedgeable_edge_pct:+.3f}")
    if mednet < cfg.min_net_hedgeable_edge_pct: fails.append(f"mednet {mednet:+.3f}<{cfg.min_net_hedgeable_edge_pct:+.3f}")
    return fails
...
def _v14_mm_failures(m: dict, cfg: Config) -> List[str]:
    ...
    if ref < cfg.min_ref_good_ratio: fails.append(f"ref {ref*100:.0f}<{cfg.min_ref_good_ratio*100:.0f}")
    if spr < cfg.min_aster_spread_pct: fails.append(f"spr {spr:.3f}<{cfg.min_aster_spread_pct:.3f}")
...
def format_scan_summary_chunks(ranked: List[Tuple[Candidate, dict]], cfg: Config) -> List[str]:
    ...
    for c, m in sorted(ranked, key=lambda cm: (str(getattr(cm[0], "target", "aster")), cm[0].symbol)):
        ...
        net = float(m.get("current_net_edge_pct", -cfg.estimated_roundtrip_fees_pct))
        ...
        raw_line = (
            f"{target_label(c)} {c.symbol} [{paths}] | gap {gross:.3f}% {side}, net {net:+.3f}% | {lag_txt} | {mm_txt}"
        )
    ...
    header = (
        f"📋 <b>V14 SCAN SUMMARY</b> — checked {len(lines)} markets\n"
        "LAG uses executable spread only: no 24h-move, midpoint-deviation or trade-count gate. MM has its own activity filter. Only failed gates are listed.\n"
    )
    ...
    for line in lines:
        ...
        if len(current) + len(piece) > max_chars and current != header:
            ...
            part += 1
            current = f"📋 <b>V14 SCAN SUMMARY continued {part}</b>\n"
        current += piece
    if current.strip():
        chunks.append(current.rstrip())
    return chunks
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V14 split LAG/MM discovery + per-run gate audit + V13 outcome/funding/execution stack)"
    )
