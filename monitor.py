#!/usr/bin/env python3
"""
V16.7 execution-economics monitor.
...
directly from this repository's Git history into memory, then installs the V16.7
...
# V16.7: ECON exits are independent from legacy LAG/MM consensus outcomes.
# A trade is marked/closed only from the exact target + hedge route stored at entry.
ECON_EXIT_LOGIC_VERSION = 3
...
_OVERLAY_MARKER = "V16.7 execution-economics monitor"
...
core.SCANNER_VERSION = "16.7"
try:
    core.USER_AGENT = "perp-inefficiency-scanner/16.7"
...
def _repair_and_apply_exact_econ_exits(state: dict, pre_open_ids: set, now_ts: float) -> bool:
    """Undo false legacy closes, then apply V16.7 exact-route exit rules."""
    changed = False
    ...
    # when its own exact route satisfies the V16.7 condition.
...
def scan_v165(
    ...
    return ranked, errors, bool(changed or exit_changed or previous_version != "16.7")
...
def build_daily_report_text(state: dict, report_date: date, tz: ZoneInfo) -> str:
    ...
    realized_funding_pct = sum(_trade_funding_pct(t, True) for t in closed)
    open_funding_pct = sum(_trade_funding_pct(t, False) for t in opened)
    total_funding_pct = realized_funding_pct + open_funding_pct
    realized_funding_usd = sum(_trade_pnl_usd(t, _trade_funding_pct(t, True)) for t in closed)
    open_funding_usd = sum(_trade_pnl_usd(t, _trade_funding_pct(t, False)) for t in opened)
    total_funding_usd = realized_funding_usd + open_funding_usd

    realized_ex_funding_pct = realized_pct_sum - realized_funding_pct
    open_ex_funding_pct = open_pct_sum - open_funding_pct
    total_ex_funding_pct = total_pct_sum - total_funding_pct
    realized_ex_funding_usd = realized_usd - realized_funding_usd
    open_ex_funding_usd = open_usd - open_funding_usd
    total_ex_funding_usd = total_usd - total_funding_usd
    ...
    lines = [
        ...
        "<b>ALL-SIGNALS PnL</b>",
        (
            f"Without funding: <b>{total_ex_funding_usd:+.4f} USD</b> | Σ {total_ex_funding_pct:+.4f}% "
            f"(realized {realized_ex_funding_usd:+.4f}, open {open_ex_funding_usd:+.4f})"
        ),
        (
            f"Funding PnL: <b>{total_funding_usd:+.4f} USD</b> | Σ {total_funding_pct:+.4f}% "
            f"(realized {realized_funding_usd:+.4f}, open {open_funding_usd:+.4f})"
        ),
        (
            f"With funding: <b>{total_usd:+.4f} USD</b> | Σ {total_pct_sum:+.4f}% "
            f"(realized {realized_usd:+.4f}, open {open_usd:+.4f})"
        ),
    ]

    if CAPITAL_SIM_ENABLED:
        # Daily report is normally generated on the first scan after midnight.
        # time.time() therefore represents the actual state used by this report.
        ...
        lines.extend([
            ...
            (
                f"Entered: <b>{cap['accepted_count']}</b> | skipped (capital busy): <b>{cap['skipped_count']}</b> | "
                f"closed/open: <b>{cap['closed_count']} / {cap['open_count']}</b>"
            ),
            f"Without funding: <b>{cap['ex_funding_usd']:+.4f} USD</b> | Σ {cap['ex_funding_pct']:+.4f}%",
            f"Funding PnL: <b>{cap['funding_usd']:+.4f} USD</b> | Σ {cap['funding_pct']:+.4f}%",
            f"With funding: <b>{cap['total_usd']:+.4f} USD</b> | Σ {cap['total_pct']:+.4f}%",
            (
                f"Cumulative portfolio equity: <b>${cap['equity_usd']:.2f}</b> ({cap['return_pct']:+.3f}% vs start) | "
                f"free ${cap['free_usd']:.2f} | reserved ${cap['reserved_usd']:.2f}"
            ),
        ])
    ...
    lines.extend([
        "",
        "PnL/exit V16.7 uses only the exact stored target + hedge route, entry/mark VWAP, taker fees and tracked funding. "
        "The capital-constrained model reserves both leg notionals from one shared pool until close; it is a conservative "
        "notional-equivalent occupancy model, not an exchange-specific leverage/margin model. Σ net is a sum of per-trade returns.",
    ])
    return "\n".join(lines)
...
def overlay_self_test() -> None:
    """Deterministic local tests for V16.7-specific overlay logic."""
    cfg = core.Config()
    state: dict = {}
    now = time.time()
    event = {
    ...
    # closed on consensus FULL, V16.7 must restore it to OPEN when the exact
    ...
    assert _repair_and_apply_exact_econ_exits(repair_state, {repair_trade["id"]}, now + 600)
    ...
    assert "Without funding" in report
    assert "CAPITAL-CONSTRAINED SIM" in report
    print("V16.7 overlay self-test OK (>0.100% NET + exact-route exits + capital occupancy + funding-separated report)")
...
def _guarded_paper_note_close(state: dict, trade: dict, *args: Any, **kwargs: Any) -> None:
    ...
    V16.7 finalizes ECON trades itself after validating the exact stored route.
...
def _guarded_update_paper_trades(*args: Any, **kwargs: Any) -> Any:
    ...
    apply the V16.7 exact-route exit rules.
    """
    ...
    if isinstance(state, dict) and pre_open_ids:
        repaired = _repair_and_apply_exact_econ_exits(state, pre_open_ids, time.time())
...
def main() -> int:
    # Run V16.7 overlay tests in addition to the original V16.4 deterministic self-test.
