...
USER_AGENT = "perp-inefficiency-scanner/12.0"
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if event_type == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V12 counter: {symbol} ACTIVE-LAG detections <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
    ...
    if event_type == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V12 counter: {symbol} ACTIVE-MM excursion events <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
...
def _attach_execution_probe_to_events(events: List[dict], candidates: List[Candidate], cfg: Config) -> List[dict]:
    by_key = {candidate_state_key(c): c for c in candidates}
    for ev in events:
...
def detect_active_lag_events(candidates: List[Candidate], cfg: Config, state: dict) -> Tuple[List[dict], bool]:
    ...
    return _attach_execution_probe_to_events(events, candidates, cfg), changed
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        f"State: <b>{m['direction']}</b>\n\n"
        "V12 scans Aster, Hyperliquid and Lighter as targets against a robust multi-venue PERP consensus and adds execution-readiness depth checks. Signal only; no orders are sent."
    )
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V12 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V12 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V12 multi-target MM/LAG + execution-readiness depth probe + ACTIVE outcomes + baseline/sanity)"
    )
