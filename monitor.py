...
def build_daily_report_text(state: dict, report_date: date, tz: ZoneInfo, cfg: Any = None) -> str:
    ...
    total_profit_locks = sum(1 for t in closed if str(t.get("close_reason") or "").startswith("ECON_TOTAL_PROFIT_LOCK"))
    negative_funding_protects = sum(1 for t in closed if str(t.get("close_reason") or "") == "ECON_TOTAL_PROFIT_PROTECT_BEFORE_NEGATIVE_FUNDING")
    funding_driven_wins = sum(1 for t in closed if str(t.get("realized_profit_driver") or "") == "FUNDING" and _trade_net_pct(t, True) > 0.0)
    ...
    lines = [
        ...
        f"Money-management exits: total-profit locks <b>{total_profit_locks}</b> | pre-negative-funding protects <b>{negative_funding_protects}</b> | funding-driven closed wins <b>{funding_driven_wins}</b>",
        "",
    ...
    lines.extend([
        "",
        "V16.10: forecast funding is timeline-based and never enters realized PnL. New trades keep a per-settlement audit trail for both exact legs; Lighter uses /api/v1/fundings hourly history. "
        "Thesis attribution remains separate from money management: SPREAD success is still measured without funding, but after a confirmed settlement a profitable total position may be locked with ECON_TOTAL_PROFIT_LOCK. "
        "Open trades refresh the next funding timeline every scan and may protect an existing positive total PnL before a known negative settlement would turn it non-positive. "
...
def overlay_self_test() -> None:
    ...
    print("V16.10 overlay self-test OK (>0.100% spread NET + settled funding audit + total-profit lock + dynamic negative-funding protection + capital occupancy)")
