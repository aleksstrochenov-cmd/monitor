#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v10.
...
USER_AGENT = "perp-inefficiency-scanner/10.0"
...
def _v10_candidate_metrics(self: Candidate, cfg: Config) -> dict:
    ...
    if self.samples:
        m["current_exec_refs"] = sorted(getattr(self.samples[-1], "exec_refs", {}).keys())
    else:
        m["current_exec_refs"] = []
    return m
            ...
            with self.lock:
                for row in stats_rows:
                    ...
                    if not symbol or market_id < 0:
                        continue
                    q = valid_quote(row.get("best_bid_price"), row.get("best_ask_price"))
                    if q:
...
def fetch_hyperliquid_book(coin: str, cfg: Config) -> Optional[Quote]:
    ...
    if not bids or not asks:
        return None
    q = valid_quote(bids[0].get("px"), asks[0].get("px")) if isinstance(bids[0], dict) and isinstance(asks[0], dict) else None
    if q:
...
def scan(
    ...
    for c in candidates:
        ...
        if m:
            ranked.append((c, m))
    ranked.sort(key=lambda x: x[1]["score"], reverse=True)
    return ranked, errors, bool(active_changed or extended_changed)
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    if "MM" in m.get("setup", ""):
        caption = (
            ...
            "V10 MM statistics use actual target-venue trades matched to the time-local robust PERP consensus."
        )
    else:
        caption = (
            f"📈 <b>{candidate.symbol} @ {label} — {m['setup']}</b>\n"
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    if "CONVERGING-LAG" in m.get("setup", ""):
        base = m.get("lag_baseline_median_gap_pct")
        base_txt = f"median {base:.3f}% ({m.get('lag_baseline_points',0)} pts)" if m.get("lag_baseline_ready") and base is not None else f"bootstrap / insufficient ({m.get('lag_baseline_points',0)})"
        lag_block = (
            f"LAG extended observation: <b>{m.get('lag_extended_observed_seconds',0):.0f}s</b> | side <b>{m.get('lag_detection_side','NONE')}</b>\n"
    ...
    return (
        ...
        f"State: <b>{m['direction']}</b>\n\n"
        "V10 scans Aster, Hyperliquid and Lighter as targets against a robust multi-venue PERP consensus. Signal only; no orders are sent."
    )
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        baseline = event.get("baseline_median_gap_pct"); btxt = "bootstrap / insufficient" if baseline is None else f"{float(baseline):.3f}%"
        return (
            ...
            f"V10 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V10 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V10 multi-target MM/LAG + robust consensus + ACTIVE signals + baseline/sanity)"
    )
