#!/usr/bin/env python3
"""Aster cross-venue algo scanner v8.
...
6) Spot prices are collected separately as diagnostics only, so normal perp/spot basis cannot create a LAG signal.
...
USER_AGENT = "aster-algo-scanner/8.0"
...
class Candidate:
    ...
    # V8 LAG verification state. Initial Stage-2 LAG detection is not sent to
    ...
    def metrics(self, cfg: "Config") -> dict:
        ...
        # V8 extended LAG convergence statistics. These are evaluated only for
...
class Config:
    ...
    # V8 extended LAG verification. Initial LAG detection is only a candidate;
...
def build_sample(
    ...
    # V8 sanity guard: identical ticker strings can still represent contracts
...
def extended_regime_observation(candidates: List[Candidate], cfg: Config, errors: List[str], state: dict) -> None:
...
def scan(cfg: Config, state: dict) -> Tuple[List[Tuple[Candidate, dict]], List[str]]:
    try:
    ...
    # V8: shared extended observation verifies both MM persistence and LAG convergence.
    extended_regime_observation(candidates, cfg, errors, state)
...
def load_state() -> dict:
    ...
    return {"last_alerts": {}, "lag_baseline": {}}
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    if "MM" in m.get("setup", ""):
        ...
        caption = (
            ...
            "V8 LAG alerts require observed convergence after detection; spot and midpoint fair remain diagnostics."
        )
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    return (
        ...
        "V8 LAG alerts are sent only after the initial executable PERP-to-PERP gap is observed for an extended window and actually converges; recent cross-run gap baseline is used to reject structural premiums/discounts when enough history exists. "
...
def self_test() -> None:
    ...
    # 6) V8: an initial LAG must actually converge during extended observation.
    ...
    print(
        "Self-test OK (V8 MM + converging LAG + baseline/sanity + thin-edge rejection + "
...
def main() -> int:
    ...
    # Cross-run LAG baseline is intentionally persisted even when no Telegram alert is sent.
    # This lets later runs distinguish a new anomaly from an always-present structural gap.
    state_changed = update_lag_baseline_state(state, [c for c, _ in ranked], cfg)
