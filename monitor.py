...
class Config:
    ...
    # V16.2: a live route is actionable only if it remains profitable after
    # applying the historically observed convergence fraction to the fresh
    # executable spread. This prevents treating 100% spread capture as the
    # default economic assumption.
    active_lag_min_convergence_adjusted_net_pct: float = float(os.getenv("ACTIVE_LAG_MIN_CONVERGENCE_ADJUSTED_NET_PCT", "0.00"))
...
def ensure_v11_state(state: dict) -> bool:
    ...
    changed = False
    if int(fnum(state.get("state_schema_version"))) < 16:
        state["state_schema_version"] = 16; changed = True
    for key, default in (
...
def _paper_note_close(state: dict, trade: dict) -> None:
    stats = state.setdefault("paper_portfolio_stats", {})
...
def _lag_convergence_adjusted_economics(event: dict, cfg: Config) -> dict:
    ...
    executable spread by the median convergence fraction observed in prior
    VERIFIED episodes. When a same-run convergence/re-expansion has no prior
    history yet, that same-run verified convergence is used as the bootstrap
    estimate. Costs remain the full round-trip taker fees plus exit-slippage
...
def _attach_execution_probe_to_events(events: List[dict], candidates: List[Candidate], cfg: Config, state: Optional[dict] = None) -> List[dict]:
    ...
    for ev in events:
        ...
        # V16.2: full-spread fresh net is not enough. A trade candidate must
        # remain positive after scaling the executable spread by its verified
        # historical convergence behaviour.
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V16.2 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V16.2 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V16.2 convergence-adjusted LAG economics + persistent paper ledger)"
    )
