#!/usr/bin/env python3
"""
V16.9 funding-timeline execution-economics monitor.
...
directly from this repository's Git history into memory, then installs the V16.9
...
# V16.9: ECON exits are independent from legacy LAG/MM consensus outcomes.
# A trade is marked/closed only from the exact target + hedge route stored at entry.
ECON_EXIT_LOGIC_VERSION = 5
...
_OVERLAY_MARKER = "V16.9 funding-timeline execution-economics monitor"
...
core.SCANNER_VERSION = "16.9"
try:
    core.USER_AGENT = "perp-inefficiency-scanner/16.9"
...
def _refresh_trade_funding_breakdown(trade: dict, cfg: Any, end_ts: float) -> None:
    ...
    is_v169 = str(trade.get("engine_version_at_entry") or "").startswith("16.9") or bool(trade.get("v169_settlement_funding"))
    try:
        ...
        if is_v169 and target_known and hedge_known:
            ...
            source = "complete-public-settlements-v169"
...
def _repair_and_apply_exact_econ_exits(state: dict, pre_open_ids: set, now_ts: float, cfg: Any = None) -> bool:
    """Undo false legacy closes, then apply V16.9 business-aware exact-route exit rules."""
    changed = False
...
def scan_v165(
    ...
    """Wrap the core scan and expose canonical V16.9 business-qualified signals."""
    previous_version = state.get("scanner_version")
    state["scanner_version"] = "16.9"
    ...
    def callback(event: dict) -> None:
        ...
        if typ == "POSITIVE-ECONOMICS":
            # Core readiness still enforces strict spread NET >0.100%. V16.9
...
def build_daily_report_text(state: dict, report_date: date, tz: ZoneInfo, cfg: Any = None) -> str:
    ...
    summed_pair_notional = sum(_trade_reserve_usd(t) for t, _closed in all_rows)
    pnl_per_summed_pair_notional_pct = (agg["total_usd"] / summed_pair_notional * 100.0) if summed_pair_notional > 0 else 0.0
    ...
    if int(agg["funding_leg_unknown_count"]) > 0:
        ...
        target_funding_line = f"Target-leg settled funding: <b>{agg['target_funding_usd']:+.4f} USD</b> | Σ trade returns {agg['target_funding_pct']:+.4f}%"
        hedge_funding_line = f"Hedge-leg settled funding: <b>{agg['hedge_funding_usd']:+.4f} USD</b> | Σ trade returns {agg['hedge_funding_pct']:+.4f}%"

    lines = [
        f"📊 <b>DAILY PAPER REPORT — {report_date.strftime('%d.%m.%Y')}</b>",
        f"Rule: fresh executable spread NET <b>&gt; {SIGNAL_NET_THRESHOLD_PCT:.3f}%</b>; known imminent negative funding can veto a marginal entry",
        f"Signals sent: <b>{len(day_signals)}</b> | funding-risk vetoes: <b>{len(day_rejects)}</b> | paper entries: <b>{paper_entries}</b> | entry failures: <b>{missing_paper}</b>",
        ...
        (f"⚠️ Extreme funding forecast warning on <b>{extreme_forecasts}</b> sent signals; realized PnL still uses settled history only." if extreme_forecasts else "Funding forecast sanity: no sent signal exceeded the configured extreme-rate warning threshold."),
        "",
        "<b>ALL-SIGNALS DIAGNOSTIC — unlimited capital; NOT portfolio ROI</b>",
        f"Gross price PnL: <b>{agg['gross_usd']:+.4f} USD</b> | Σ trade returns {agg['gross_pct']:+.4f}%",
        f"Taker fees: <b>-{agg['fees_usd']:.4f} USD</b> | Σ trade returns -{agg['fees_pct']:.4f}%",
        f"Spread NET (price-fees): <b>{agg['spread_usd']:+.4f} USD</b> | Σ trade returns {agg['spread_pct']:+.4f}%",
        target_funding_line,
        f"NET settled funding: <b>{agg['funding_usd']:+.4f} USD</b> | Σ trade returns {agg['funding_pct']:+.4f}%",
        f"Independent-trade PnL: <b>{agg['total_usd']:+.4f} USD</b> (realized {realized_usd:+.4f}, open MTM {open_usd:+.4f})",
        f"Σ per-trade return: <b>{agg['total_pct']:+.4f}%</b> — diagnostic only, <b>NOT</b> account ROI",
        f"Summed pair notional across entries: <b>${summed_pair_notional:,.2f}</b> | PnL / summed pair notional: <b>{pnl_per_summed_pair_notional_pct:+.4f}%</b>",
        "",
        "<b>BY BUSINESS THESIS</b>",
    ]
    if hedge_funding_line is not None:
        idx = next((i for i, x in enumerate(lines) if x.startswith("NET settled funding:")), len(lines))
    ...
    for thesis in ("SPREAD", "SPREAD+FUNDING", "FUNDING", "LEGACY"):
        ...
        lines.append(
            ...
            f"spread {a['spread_usd']:+.4f} | settled funding {a['funding_usd']:+.4f} | total <b>{a['total_usd']:+.4f} USD</b>"
        )

    if CAPITAL_SIM_ENABLED:
        cap = _capital_constrained_snapshot(state, report_date, tz, time.time())
        cap_funding_split = (
            f"Target settled funding: <b>{cap['target_funding_usd']:+.4f} USD</b> | hedge settled funding: <b>{cap['hedge_funding_usd']:+.4f} USD</b>"
        ...
        lines.extend([
            ...
            f"Report-date accepted cohort — gross price: <b>{cap['gross_usd']:+.4f} USD</b> | fees: <b>-{cap['fees_usd']:.4f} USD</b>",
            f"Report-date spread NET: <b>{cap['ex_funding_usd']:+.4f} USD</b>",
            cap_funding_split,
            f"Report-date NET settled funding: <b>{cap['funding_usd']:+.4f} USD</b>",
            f"Report-date accepted-cohort PnL: <b>{cap['total_usd']:+.4f} USD</b>",
            f"Portfolio identity: free <b>${cap['free_usd']:.2f}</b> + reserved <b>${cap['reserved_usd']:.2f}</b> + open MTM <b>{cap['open_mtm_usd']:+.2f}</b> = equity <b>${cap['equity_usd']:.2f}</b>",
            f"Cumulative portfolio ROI vs ${cap['start_balance_usd']:,.0f} start: <b>{cap['return_pct']:+.3f}%</b>",
        ])
    ...
    lines.extend([
        "",
        "V16.9: forecast funding is timeline-based and never enters realized PnL. New V16.9 trades use signed public settled funding history on both exact legs; Lighter uses /api/v1/fundings with hourly observations. "
        "SPREAD exits ignore funding; SPREAD+FUNDING may exit on spread alone or after an actual settlement on the combined target. "
        "ALL-SIGNALS percentage sums are diagnostics, not ROI; portfolio ROI is shown only in the capital-constrained block.",
    ])
    return "\n".join(lines)
...
def overlay_self_test() -> None:
    """Deterministic local tests for V16.9-specific overlay logic."""
    cfg = core.Config()
    state: dict = {}
    now = time.time()
    event = {
    ...
    # closed on consensus FULL, V16.9 must restore it to OPEN when the exact
    ...
    assert "Signals sent: <b>1</b>" in report
    assert "ALL-SIGNALS DIAGNOSTIC" in report
    assert "Spread NET" in report
    assert "Target-leg settled funding" in report
    assert "BY BUSINESS THESIS" in report
    assert "CAPITAL-CONSTRAINED SIM" in report
    assert "NOT portfolio ROI" in report
    assert "Cumulative portfolio ROI" in report
    print("V16.9 overlay self-test OK (>0.100% spread NET + timeline funding + settled Lighter history + negative-funding guard + capital occupancy)")
...
def _guarded_paper_note_close(state: dict, trade: dict, *args: Any, **kwargs: Any) -> None:
    ...
    V16.9 finalizes ECON trades itself after validating the exact stored route.
...
def _guarded_update_paper_trades(*args: Any, **kwargs: Any) -> Any:
    ...
    apply the V16.9 business-aware exact-route exit rules.
    """
...
def main() -> int:
    # Run V16.9 overlay tests in addition to the original V16.4 deterministic self-test.
