#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v16.
...
USER_AGENT = "perp-inefficiency-scanner/16.0"
...
class Config:
    ...
    # V16.3: a live route is actionable only if it remains profitable after
...
def ensure_v11_state(state: dict) -> bool:
    ...
    changed = False
    if int(fnum(state.get("state_schema_version"))) < 17:
        state["state_schema_version"] = 17; changed = True
    for key, default in (
...
def _attach_execution_probe_to_events(events: List[dict], candidates: List[Candidate], cfg: Config, state: Optional[dict] = None) -> List[dict]:
    ...
    for ev in events:
        ...
        # V16.3: if the structural baseline is READY, re-check it against the
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V16.3 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V16.3 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def self_test() -> None:
    ...
    print(
        "Self-test OK (V16.3 route-specific convergence economics + baseline gate + persistent paper ledger)"
    )
...
def main() -> int:
    ...
    state_changed = bool(scan_state_changed or callback_state_changed or paper_mark_changed)
