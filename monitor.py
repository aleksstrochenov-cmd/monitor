#!/usr/bin/env python3
"""Multi-target perp inefficiency scanner v16.4.

Purpose
-------
Read PUBLIC market data only. No exchange API keys and no trading.

V16 separates market-behaviour research, actionable signals and persistent paper accounting:
1) Broad multi-target discovery. LAG is based on executable cross-venue bid/ask
   spreads and is independent of 24h coin movement. MM keeps its own activity
   discovery path.
2) A shared confirmation window measures gross executable spread persistence,
   reference quality and real target-trade excursions. Generic flat fees do not
   decide whether a spread exists.
3) LAG candidates can enter extended observation to measure convergence,
   re-expansion, BBO coverage and historical structural-gap baselines.
4) MM candidates enter extended observation after the first real excursion;
   final MM quality still uses excursion count, clean reversion rate and speed.
5) Every entry-style Telegram signal is separately gated by a fresh public-depth
   execution probe at the configured USD notional. The probe chooses the best
   real hedge venue by NET economics after target/hedge VWAP, conservative
   open+close taker fees on both legs, and an exit-slippage reserve.
6) Broad POSITIVE-ECONOMICS Telegram alerts are disabled by default. Actionable
   LAG requires prior/same-run convergence evidence; actionable MM requires a
   live excursion/reversion regime. Both still require fresh positive economics.
7) Every actionable ACTIVE-LAG / ACTIVE-MM-EXCURSION can open a persistent
   normalized paper trade in state/state.json. The exact target+hedge route,
   entry VWAPs, fees, funding and later executable close VWAPs are preserved.
8) On later GitHub Actions runs the scanner reloads those open paper trades,
   checks the SAME route, marks convergence/reversion, closes by deterministic
   rules, and accumulates retrospective PnL in units of one-leg capital X.
9) MM episode statistics are persisted separately from paper trades, including
   clean/non-clean reversion outcomes and bounded peak/reversion samples.
10) A versioned PAPER PERFORMANCE REPORT is built from closed paper trades,
    persisted in state/state.json with report history, and can be delivered to
    Telegram periodically. It compares predicted vs realized economics by exact
    target/symbol/side/hedge route.

This is designed for scheduled GitHub Actions runs. It is a research/signal
scanner for manual inspection, not an order-routing bot or sub-second HFT engine.
"""
...
PAPER_REPORT_SCHEMA_VERSION = 1
USER_AGENT = f"perp-inefficiency-scanner/{SCANNER_VERSION}"
...
class Candidate:
    ...
    def metrics(self, cfg: "Config") -> dict:
        ...
        # V15: persistence measures the SPREAD itself, not profitability under a
        # single global fee reserve. Economics is evaluated later from fresh
        # executable depth with venue-specific round-trip taker fees.
        long_hit_ratio = (sum(
        ...
        lag_score = min(
            100.0,
            25.0 * min(1.0, current_side_edge_positive / max(cfg.min_current_hedgeable_edge_pct, 1e-9))
            + 25.0 * persistent_exec_hit_ratio
            + 15.0 * min(1.0, max(0.0, persistent_median_edge) / max(cfg.min_hedgeable_edge_pct, 1e-9))
            + 15.0 * min(1.0, max(0.0, persistent_max_edge) / max(cfg.min_hedgeable_edge_pct * 1.5, 1e-9))
            + 10.0 * ref_good_ratio
        ...
        # V15: RAW-LAG means a real executable cross-venue spread was persistent.
        # It intentionally does NOT depend on a generic fee reserve or target
        # internal spread. Positive trade economics is a later, fresh-depth gate.
        raw_lag_confirmed = (
...
class Config:
    ...
    min_hedgeable_edge_pct: float = float(
        os.getenv("MIN_HEDGEABLE_EDGE_PCT", os.getenv("MIN_EXECUTABLE_EDGE_PCT", "0.10"))
    )
    min_current_hedgeable_edge_pct: float = float(
        os.getenv("MIN_CURRENT_HEDGEABLE_EDGE_PCT", os.getenv("MIN_CURRENT_EXECUTABLE_EDGE_PCT", "0.10"))
    )
    min_net_hedgeable_edge_pct: float = float(os.getenv("MIN_NET_HEDGEABLE_EDGE_PCT", "0.00"))
    min_hedgeable_edge_hit_ratio: float = float(
        os.getenv("MIN_HEDGEABLE_EDGE_HIT_RATIO", os.getenv("MIN_EXEC_EDGE_HIT_RATIO", "0.30"))
    )
    ...
    max_candidates: int = int(os.getenv("MAX_CANDIDATES", "80"))

    # V14 discovery split. LAG and MM no longer share the same hot-coin gate.
    # LAG discovery intentionally does NOT require a minimum 24h move; a spread
    # either exists on executable BBOs or it does not. The per-target/global caps
    # keep the run bounded. MM discovery can still favor moving/liquid markets.
    lag_discovery_share: float = float(os.getenv("LAG_DISCOVERY_SHARE", "0.75"))
    lag_discovery_edge_share: float = float(os.getenv("LAG_DISCOVERY_EDGE_SHARE", "0.60"))
    lag_discovery_rotation_enabled: bool = os.getenv("LAG_DISCOVERY_ROTATION_ENABLED", "true").strip().lower() in {"1","true","yes","on"}
    mm_discovery_min_24h_move_pct: float = float(os.getenv("MM_DISCOVERY_MIN_24H_MOVE_PCT", "5.0"))
    mm_discovery_min_quote_volume24h: float = float(os.getenv("MM_DISCOVERY_MIN_QUOTE_VOLUME_24H", os.getenv("MIN_ASTER_QUOTE_VOLUME_24H", "50000")))

    # V14 per-run Telegram audit. Every selected market gets one compact line
    # showing the current gap and the exact LAG/MM gates that failed.
    scan_summary_enabled: bool = os.getenv("SCAN_SUMMARY_ENABLED", "true").strip().lower() in {"1","true","yes","on"}
    scan_summary_max_chars: int = int(os.getenv("SCAN_SUMMARY_MAX_CHARS", "3600"))
    ...
    active_lag_min_gross_edge_pct: float = float(os.getenv("ACTIVE_LAG_MIN_GROSS_EDGE_PCT", "0.10"))
    active_lag_min_net_edge_pct: float = float(os.getenv("ACTIVE_LAG_MIN_NET_EDGE_PCT", "0.00"))
    active_lag_min_hit_ratio: float = float(os.getenv("ACTIVE_LAG_MIN_HIT_RATIO", "0.30"))
    active_lag_max_reference_disagreement_pct: float = float(os.getenv("ACTIVE_LAG_MAX_REFERENCE_DISAGREEMENT_PCT", "0.20"))
    # V16.4: a live route is actionable only if it remains profitable after
    # applying the historically observed convergence fraction to the fresh
    # executable spread. A small positive margin is required because +0.001%
    # style signals are economically indistinguishable from microstructure noise.
    active_lag_min_convergence_adjusted_net_pct: float = float(os.getenv("ACTIVE_LAG_MIN_CONVERGENCE_ADJUSTED_NET_PCT", "0.02"))
    # Once at least this many exact target/symbol/side/hedge paper episodes exist,
    # use their route-specific convergence instead of the broader consensus profile.
    active_lag_min_route_profile_episodes: int = int(os.getenv("ACTIVE_LAG_MIN_ROUTE_PROFILE_EPISODES", "1"))
    ...
    active_mm_min_prior_excursions: int = int(os.getenv("ACTIVE_MM_MIN_PRIOR_EXCURSIONS", "1"))
    ...
    active_mm_require_live_excursion: bool = os.getenv("ACTIVE_MM_REQUIRE_LIVE_EXCURSION", "true").strip().lower() in {"1","true","yes","on"}
    ...
    execution_min_net_edge_pct: float = float(os.getenv("EXECUTION_MIN_NET_EDGE_PCT", "0.01"))
    ...
    # V15 venue-specific economics. Defaults are conservative public/base-tier
    # taker rates for API-style execution as of 2026-09-12. They are fully
    # overrideable from Actions env for the user's actual VIP/account tier.
    fee_taker_aster_pct: float = float(os.getenv("FEE_TAKER_ASTER_PCT", "0.040"))
    fee_taker_hyperliquid_pct: float = float(os.getenv("FEE_TAKER_HYPERLIQUID_PCT", "0.045"))
    fee_taker_lighter_pct: float = float(os.getenv("FEE_TAKER_LIGHTER_PCT", "0.000"))
    fee_taker_bitget_pct: float = float(os.getenv("FEE_TAKER_BITGET_PCT", "0.060"))
    fee_taker_mexc_pct: float = float(os.getenv("FEE_TAKER_MEXC_PCT", "0.080"))
    fee_taker_bybit_pct: float = float(os.getenv("FEE_TAKER_BYBIT_PCT", "0.055"))
    fee_taker_bybit_innovation_pct: float = float(os.getenv("FEE_TAKER_BYBIT_INNOVATION_PCT", "0.110"))
    fee_taker_bybit_premarket_pct: float = float(os.getenv("FEE_TAKER_BYBIT_PREMARKET_PCT", "0.100"))
    fee_taker_dydx_pct: float = float(os.getenv("FEE_TAKER_DYDX_PCT", "0.050"))
    fee_taker_edgex_pct: float = float(os.getenv("FEE_TAKER_EDGEX_PCT", "0.038"))

    # A signal is delivered only when fresh $notional VWAP economics stay
    # positive after four taker executions (open+close on both legs) plus a
    # reserve for EXIT slippage equal to the observed entry slippage by default.
    economic_signals_enabled: bool = os.getenv("ECONOMIC_SIGNALS_ENABLED", "true").strip().lower() in {"1","true","yes","on"}
    economic_probe_min_gross_pct: float = float(os.getenv("ECONOMIC_PROBE_MIN_GROSS_PCT", "0.08"))
    economic_min_net_edge_pct: float = float(os.getenv("ECONOMIC_MIN_NET_EDGE_PCT", "0.01"))
    economic_exit_slippage_multiplier: float = float(os.getenv("ECONOMIC_EXIT_SLIPPAGE_MULTIPLIER", "1.0"))
    economic_probe_max_candidates: int = int(os.getenv("ECONOMIC_PROBE_MAX_CANDIDATES", "160"))
    economic_signal_cooldown_minutes: int = int(os.getenv("ECONOMIC_SIGNAL_COOLDOWN_MINUTES", "15"))
    execution_max_hedge_fair_deviation_pct: float = float(os.getenv("EXECUTION_MAX_HEDGE_FAIR_DEVIATION_PCT", "0.30"))

    # V16 persistent paper ledger. `paper_notional_x` is a normalized one-leg
    # capital unit; executable prices are sampled at `paper_probe_notional_usd`
    # because slippage is size-dependent. The reported PnL remains % of X.
    paper_trading_enabled: bool = os.getenv("PAPER_TRADING_ENABLED", "true").strip().lower() in {"1","true","yes","on"}
    paper_probe_notional_usd: float = float(os.getenv("PAPER_PROBE_NOTIONAL_USD", os.getenv("EXECUTION_MIN_NOTIONAL_USD", "100")))
    paper_notional_x: float = float(os.getenv("PAPER_NOTIONAL_X", "1.0"))
    paper_enforce_venue_capital: bool = os.getenv("PAPER_ENFORCE_VENUE_CAPITAL", "true").strip().lower() in {"1","true","yes","on"}
    paper_venue_capacity_x: float = float(os.getenv("PAPER_VENUE_CAPACITY_X", "1.0"))
    paper_capital_fraction_per_trade: float = float(os.getenv("PAPER_CAPITAL_FRACTION_PER_TRADE", "1.0"))
    paper_min_entry_net_edge_pct: float = float(os.getenv("PAPER_MIN_ENTRY_NET_EDGE_PCT", "0.0"))
    paper_checkpoint_minutes: Tuple[float, ...] = tuple(sorted(set(
        float(x.strip()) for x in os.getenv("PAPER_CHECKPOINT_MINUTES", "5,30,120,360,1440").split(",") if x.strip()
    )))
    paper_lag_close_convergence_fraction: float = float(os.getenv("PAPER_LAG_CLOSE_CONVERGENCE_FRACTION", "0.80"))
    paper_lag_close_edge_pct: float = float(os.getenv("PAPER_LAG_CLOSE_EDGE_PCT", "0.05"))
    paper_lag_max_hold_minutes: float = float(os.getenv("PAPER_LAG_MAX_HOLD_MINUTES", "1440"))
    paper_mm_close_convergence_fraction: float = float(os.getenv("PAPER_MM_CLOSE_CONVERGENCE_FRACTION", "0.80"))
    paper_mm_close_edge_pct: float = float(os.getenv("PAPER_MM_CLOSE_EDGE_PCT", os.getenv("REVERSION_BAND_PCT", "0.08")))
    paper_mm_max_hold_seconds: float = float(os.getenv("PAPER_MM_MAX_HOLD_SECONDS", "300"))
    paper_mm_require_live_excursion: bool = os.getenv("PAPER_MM_REQUIRE_LIVE_EXCURSION", "true").strip().lower() in {"1","true","yes","on"}
    paper_max_open_trades: int = int(os.getenv("PAPER_MAX_OPEN_TRADES", "100"))
    # Mark OPEN paper trades concurrently. Without this, persistent positions are
    # refreshed one-by-one and run time grows roughly linearly as the ledger fills.
    paper_mark_workers: int = int(os.getenv("PAPER_MARK_WORKERS", "8"))
    paper_max_closed_trades: int = int(os.getenv("PAPER_MAX_CLOSED_TRADES", "5000"))
    paper_max_skipped_signals: int = int(os.getenv("PAPER_MAX_SKIPPED_SIGNALS", "1000"))
    paper_mm_profile_sample_cap: int = int(os.getenv("PAPER_MM_PROFILE_SAMPLE_CAP", "100"))

    # V16.4 versioned retrospective report. A new report version is persisted
    # whenever its underlying paper ledger changes. Telegram delivery is
    # throttled independently so state can evolve without message spam.
    paper_report_enabled: bool = os.getenv("PAPER_REPORT_ENABLED", "true").strip().lower() in {"1","true","yes","on"}
    paper_report_send_telegram: bool = os.getenv("PAPER_REPORT_SEND_TELEGRAM", "true").strip().lower() in {"1","true","yes","on"}
    paper_report_interval_minutes: int = int(os.getenv("PAPER_REPORT_INTERVAL_MINUTES", "360"))
    paper_report_min_closed_trades: int = int(os.getenv("PAPER_REPORT_MIN_CLOSED_TRADES", "1"))
    paper_report_top_routes: int = int(os.getenv("PAPER_REPORT_TOP_ROUTES", "5"))
    paper_report_history_limit: int = int(os.getenv("PAPER_REPORT_HISTORY_LIMIT", "50"))

    # V15 MM candidates enter extended observation after the first real
    # excursion; the strict >=3/clean-reversion requirements remain confirmation
    # rules, not admission rules.
    extended_mm_trigger_excursions: int = int(os.getenv("EXTENDED_MM_TRIGGER_EXCURSIONS", "1"))
    ...
    def process_active_mm_events(force_emit: bool = False) -> None:
        ...
        for c in selected_mm:
            ...
            if pending and (force_emit or elapsed_since_emit >= cfg.active_mm_alert_batch_seconds):
                ...
                # V15: an MM event is a Telegram trade signal only when a fresh
                # hedged pair route has positive conservative economics.
                try:
                    payload.update(execution_readiness_probe(c, c.metrics(cfg), str(latest["action"]), cfg))
                except Exception as e:
                    payload.update({"execution_status": f"PROBE_ERROR:{type(e).__name__}", "execution_ready": False})
                if active_signal_callback is not None and payload.get("execution_ready"):
    ...
    try:
        if STATE_PATH.exists():
            ...
            if isinstance(data, dict):
                ...
                data.setdefault("paper_trades_open", {})
                data.setdefault("paper_trades_closed", [])
                data.setdefault("paper_portfolio_stats", {})
                data.setdefault("paper_signals_skipped", [])
                data.setdefault("mm_profiles", {})
                data.setdefault("lag_route_profiles", {})
                data.setdefault("paper_performance_report", {})
                data.setdefault("scanner_version", SCANNER_VERSION)
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if event_type == "ACTIVE-LAG":
        ...
        return (
            ...
            f"V15 counter: {symbol} ACTIVE-LAG detections <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
    ...
    if event_type == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"V15 counter: {symbol} ACTIVE-MM excursion events <b>{symbol_total}</b> | all symbols <b>{total}</b>\n\n"
...
def detect_active_lag_events(candidates: List[Candidate], cfg: Config, state: dict) -> Tuple[List[dict], bool]:
    ...
    for c in candidates:
        ...
        event = {
            ...
            "baseline_median_gap_pct": baseline_median,
            "baseline_required_gap_pct": baseline_required,
            "historical_median_convergence_fraction": profile.get("median_max_convergence_fraction"),
            "same_run_convergence_fraction": None,
            **profile,
        }
        events.append(event)
...
def extended_regime_observation(
    ...
    for c in candidates:
        ...
        c.run_baseline_edge_pct = max(0.0, float(m.get("persistent_median_executable_edge_pct", 0.0)))
        if (
            int(m.get("excursion_count", 0)) >= cfg.extended_mm_trigger_excursions
            and float(m.get("ref_good_ratio", 0.0)) >= cfg.min_ref_good_ratio
        ):
            rank = float(m.get("mm_score", 0)) + min(20.0, float(m.get("excursion_count", 0)) * 3.0) + float(m.get("clean_reversion_rate", 0)) * 10
            initial_mm.append((rank, c, m))
        if bool(m.get("raw_lag_confirmed")):
            c.lag_verification_status = "not_selected"
            rank = float(m.get("lag_score", 0)) + float(m.get("persistent_exec_hit_ratio", 0)) * 15 + min(15.0, max(0.0, float(m.get("persistent_median_executable_edge_pct", 0))) * 50)
            initial_lag.append((rank, c, m))
    initial_mm.sort(key=lambda x: x[0], reverse=True)
    initial_lag.sort(key=lambda x: x[0], reverse=True)
    selected_mm = [c for _, c, _ in initial_mm[: max(0, cfg.extended_mm_max_candidates)]] if cfg.extended_mm_enabled else []
    ...
    def process_active_mm_events(force_emit: bool = False) -> None:
        ...
        for c in selected_mm:
            ...
            if int(context.get("excursion_count", 0)) < cfg.active_mm_min_prior_excursions:
                continue
            window_start = start - max(0.0, cfg.active_mm_initial_lookback_seconds)
            new_events = []
            for exc in c.excursions:
            ...
            if new_events:
                ...
                record_signal_stats(state, "MM-EXCURSION-OBSERVED", skey, len(new_events), side_counts)
                state_changed = True
            ...
            if pending and (force_emit or elapsed >= cfg.active_mm_alert_batch_seconds):
                # A batch can contain opposite-direction excursions. Economics
                # must therefore be proven independently for each action; one
                # profitable SHORT must never "bless" a negative LONG event.
                metrics_now = c.metrics(cfg)
                for action in ("SHORT", "LONG"):
                    side_events = [e for e in pending if str(e.get("action")) == action]
                    if not side_events:
                        continue
                    peaks = [float(e["peak_pct"]) for e in side_events]
                    latest = max(side_events, key=lambda e: float(e["start_ts"]))
                    # V16 entry-style MM alerts must still be actionable when
                    # emitted. Historical/reverted excursions remain in MM
                    # research profiles but are not sent as trade candidates.
                    if cfg.active_mm_require_live_excursion and bool(latest.get("reverted")):
                        continue
                    payload = {
                        "type": "ACTIVE-MM-EXCURSION", "symbol": c.symbol, "state_key": skey,
                        "target": getattr(c, "target", "aster"), "target_label": target_label(c),
                        "ts": now_ts, "events": list(side_events), "event_count": len(side_events),
                        "long_count": sum(e["action"] == "LONG" for e in side_events),
                        "short_count": sum(e["action"] == "SHORT" for e in side_events),
                        "median_peak_pct": statistics.median(peaks) if peaks else 0.0,
                        "max_peak_pct": max(peaks) if peaks else 0.0,
                    ...
                    try:
                        payload.update(cached_execution_probe(c, metrics_now, action, cfg, force_fresh=True))
                    except Exception as e:
                        payload.update({"execution_status": f"PROBE_ERROR:{type(e).__name__}", "execution_ready": False})
                    if not payload.get("execution_ready"):
                        continue
                    # Defensive invariant in addition to execution_ready.
                    if fnum(payload.get("execution_fresh_net_edge_pct"), -999.0) <= 0.0:
                        continue
                    side_counts = {"LONG": len(side_events) if action == "LONG" else 0,
                                   "SHORT": len(side_events) if action == "SHORT" else 0}
                    record_signal_stats(state, "ACTIVE-MM-EXCURSION", skey, len(side_events), side_counts)
                    for pe in side_events:
                        eid = f"{skey}:{pe['direction']}:{int(float(pe['start_ts']) * 1000)}"
                        orow = state.setdefault("active_mm_outcomes", {}).get(eid)
                        if isinstance(orow, dict):
                            orow["signal_sent"] = True
                            orow["execution_fresh_gross_edge_pct"] = payload.get("execution_fresh_gross_edge_pct")
                            orow["execution_fresh_net_edge_pct"] = payload.get("execution_fresh_net_edge_pct")
                            orow["execution_roundtrip_fees_pct"] = payload.get("execution_roundtrip_fees_pct")
                            orow["execution_best_external_venue"] = payload.get("execution_best_external_venue")
                    if active_signal_callback is not None:
                        active_signal_callback(payload)
                pending.clear()
...
# ------------------------------ V14 long-horizon LAG outcomes + funding ------------------------------
...
def _lag_pair_pnl_if_closed_pct(row: dict, cfg: Config) -> float:
    ...
    entry_cost = fnum(row.get("entry_roundtrip_cost_pct"), cfg.estimated_roundtrip_fees_pct)
    return (entry - current) - entry_cost + funding


def ensure_v11_state(state: dict) -> bool:
    """V16 state migration; legacy rows remain as an audit trail."""
    changed = False
    if int(fnum(state.get("state_schema_version"))) < 18:
        state["state_schema_version"] = 18; changed = True
    for key, default in (
        ("active_lag_outcomes", {}), ("active_mm_outcomes", {}),
        ("outcome_stats", {}), ("reference_health", {}),
        ("paper_trades_open", {}), ("paper_trades_closed", []),
        ("paper_portfolio_stats", {}), ("paper_signals_skipped", []), ("mm_profiles", {}),
        ("lag_route_profiles", {}), ("paper_performance_report", {}),
    ):
        if not isinstance(state.get(key), type(default)):
            state[key] = default; changed = True
        elif key not in state:
            state[key] = default; changed = True
    if state.get("scanner_version") != SCANNER_VERSION:
        state["scanner_version"] = SCANNER_VERSION; changed = True
    return changed
...
def start_active_lag_outcome(state: dict, event: dict, cfg: Config) -> bool:
    ...
    bucket[event_id] = {
        ...
        "entry_roundtrip_fees_pct": fnum(event.get("execution_roundtrip_fees_pct"), cfg.estimated_roundtrip_fees_pct),
        "entry_exit_slippage_reserve_pct": fnum(event.get("execution_exit_slippage_reserve_pct"), 0.0),
        "entry_roundtrip_cost_pct": fnum(event.get("execution_roundtrip_fees_pct"), cfg.estimated_roundtrip_fees_pct) + fnum(event.get("execution_exit_slippage_reserve_pct"), 0.0),
        "entry_net_edge_pct": fnum(event.get("execution_fresh_net_edge_pct"), fnum(event.get("net_edge_pct"))),
        "horizons": {}, "funding": {}, "version": 15,
    }
...
def finalize_mm_outcomes(state: dict, cfg: Config, now_ts: Optional[float]=None) -> Tuple[List[dict], bool]:
    ...
    for eid,row in list(bucket.items()):
        ...
        if status in {"CLEAN_REVERTED","REVERTED_NOT_CLEAN","STALE_UNRESOLVED"} and not row.get("profile_counted"):
            # V16 research profile counts every observed MM excursion, including
            # excursions that were not economically actionable Telegram signals.
            _update_mm_profile_from_outcome(state, row, cfg)
            row["profile_counted"] = True; changed = True
        if status in {"CLEAN_REVERTED","REVERTED_NOT_CLEAN","STALE_UNRESOLVED"} and not row.get("counted"):
            # Outcome alerts/stats belong only to MM events that actually passed
            # the fresh positive-economics gate and were sent as signals.
            if row.get("signal_sent"):
                ...
                st["last_ts"]=int(now_ts); emitted.append({"type":"ACTIVE-MM-OUTCOME", **row})
            row["counted"]=True; changed=True
    if len(bucket)>500:
        ordered=sorted(bucket.items(), key=lambda kv:fnum(kv[1].get("start_ts")) if isinstance(kv[1],dict) else 0)
...
def venue_taker_fee_pct(venue: str, cfg: Config) -> float:
    v = str(venue or "").lower()
    aliases = {
        "aster": cfg.fee_taker_aster_pct, "aster-perp": cfg.fee_taker_aster_pct,
        "hyperliquid": cfg.fee_taker_hyperliquid_pct, "hyperliquid-perp": cfg.fee_taker_hyperliquid_pct,
        "lighter": cfg.fee_taker_lighter_pct, "lighter-perp": cfg.fee_taker_lighter_pct,
        "bitget": cfg.fee_taker_bitget_pct, "bitget-perp": cfg.fee_taker_bitget_pct,
        "mexc": cfg.fee_taker_mexc_pct, "mexc-perp": cfg.fee_taker_mexc_pct,
        "bybit": cfg.fee_taker_bybit_pct, "bybit-perp": cfg.fee_taker_bybit_pct,
        "dydx": cfg.fee_taker_dydx_pct, "dydx-perp": cfg.fee_taker_dydx_pct,
        "edgex": cfg.fee_taker_edgex_pct, "edgex-perp": cfg.fee_taker_edgex_pct,
    }
    return max(0.0, float(aliases.get(v, 999.0)))


def pair_roundtrip_taker_fees_pct(target: str, hedge_venue: str, cfg: Config) -> float:
    """Open+close both legs, assuming taker fills on all four executions."""
    tf = venue_taker_fee_pct(target, cfg)
    hf = venue_taker_fee_pct(hedge_venue, cfg)
    if tf >= 900 or hf >= 900:
        return math.inf
    return 2.0 * (tf + hf)
...
_DEPTH_EXEC_VENUES = {
    "aster-perp", "hyperliquid-perp", "lighter-perp",
    "bitget-perp", "mexc-perp", "bybit-perp", "dydx-perp",
}


def minimum_possible_roundtrip_fee_pct(target: str, refs: Iterable[str], cfg: Config) -> float:
    """Best conservative four-taker fee route among currently executable refs.

    This is only a cheap pre-probe bound. The actual signal still requires a
    fresh real-order-book VWAP probe, full fill at the requested notional, and a
    positive net result after fees plus exit-slippage reserve.
    """
    target_ref = f"{str(target).lower()}-perp"
    fees = []
    for venue in set(str(x).lower() for x in refs or []):
        if venue == target_ref or venue not in _DEPTH_EXEC_VENUES:
            continue
        fee = pair_roundtrip_taker_fees_pct(target, venue, cfg)
        if math.isfinite(fee):
            fees.append(fee)
    return min(fees) if fees else math.inf


def _candidate_probe_cache(c: Candidate) -> dict:
    cache = getattr(c, "_economic_probe_cache", None)
    if not isinstance(cache, dict):
        cache = {}
        setattr(c, "_economic_probe_cache", cache)
    return cache


def cached_execution_probe(c: Candidate, m: dict, side: str, cfg: Config, *, force_fresh: bool = False) -> dict:
    """Use a very short-lived per-candidate cache to avoid duplicate API depth calls."""
    side = str(side).upper()
    cache = _candidate_probe_cache(c)
    row = cache.get(side)
    now = time.time()
    max_age = max(1.0, float(cfg.execution_max_book_age_seconds))
    if not force_fresh and isinstance(row, dict) and now - fnum(row.get("execution_probe_ts")) <= max_age:
        return dict(row)
    probe = execution_readiness_probe(c, m, side, cfg)
    cache[side] = dict(probe)
    return probe


def best_positive_execution_probe(
    c: Candidate, m: dict, cfg: Config, preferred_side: Optional[str] = None, *, force_fresh: bool = False
) -> Tuple[Optional[str], Optional[dict]]:
    """Return the best fresh positive route for this target market, if any."""
    side_order: List[str] = []
    for side in (preferred_side, m.get("persistent_edge_side"), m.get("current_edge_side")):
        side = str(side or "").upper()
        if side in {"LONG", "SHORT"} and side not in side_order:
            side_order.append(side)
    if c.excursions:
        last_exc = max(c.excursions, key=lambda x: x.start_ts)
        exc_side = "SHORT" if last_exc.direction == "ABOVE" else "LONG"
        if exc_side not in side_order:
            side_order.append(exc_side)
    for side in ("SHORT", "LONG"):
        if side not in side_order:
            side_order.append(side)

    refs = list(dict.fromkeys(list(m.get("current_exec_refs", [])) + list(m.get("current_refs", []))))
    theoretical_fee = minimum_possible_roundtrip_fee_pct(str(getattr(c, "target", "aster")), _DEPTH_EXEC_VENUES, cfg)
    best_side: Optional[str] = None
    best_probe: Optional[dict] = None
    sample = c.samples[-1] if c.samples else None
    for side in side_order:
        # Cheap bound avoids unnecessary depth calls while never excluding a
        # route that could be positive under the configured taker fees.
        if sample is not None:
            potential = max(
                fnum(sample.hedge_short_edge_pct if side == "SHORT" else sample.hedge_long_edge_pct, -999),
                fnum(sample.short_edge_pct if side == "SHORT" else sample.long_edge_pct, -999),
                fnum(sample.best_hedge_short_edge_pct if side == "SHORT" else sample.best_hedge_long_edge_pct, -999),
            )
            required = max(cfg.economic_probe_min_gross_pct, theoretical_fee + cfg.economic_min_net_edge_pct)
            if math.isfinite(required) and potential + 1e-12 < required:
                continue
        try:
            probe = cached_execution_probe(c, m, side, cfg, force_fresh=force_fresh)
        except Exception:
            continue
        if not probe.get("execution_ready"):
            continue
        if best_probe is None or fnum(probe.get("execution_fresh_net_edge_pct"), -999) > fnum(best_probe.get("execution_fresh_net_edge_pct"), -999):
            best_side, best_probe = side, probe
    return best_side, best_probe


# ------------------------------ V12/V15 execution readiness ------------------------------
...
_BYBIT_FEE_CLASS_CACHE: Dict[str, Tuple[float, float]] = {}


def _bybit_taker_fee_for_symbol(symbol: str, cfg: Config) -> float:
    """Conservative public taker fee for a Bybit linear instrument.

    Normal VIP0 perpetual/futures use the base rate, while Innovation Zone and
    Pre-Market perpetuals have higher published base taker rates. If metadata
    cannot be verified, use the highest configured rate rather than risk a
    false-positive economics signal.
    """
    now = time.time()
    cached = _BYBIT_FEE_CLASS_CACHE.get(symbol)
    if cached and now - cached[0] < 1800:
        return cached[1]
    fallback = max(cfg.fee_taker_bybit_pct, cfg.fee_taker_bybit_innovation_pct, cfg.fee_taker_bybit_premarket_pct)
    fee = fallback
    try:
        data = get_json(
            f"{BYBIT_BASE}/v5/market/instruments-info", cfg.request_timeout_seconds,
            params={"category": "linear", "symbol": symbol},
        )
        rows = ((data or {}).get("result", {}) or {}).get("list", []) if isinstance(data, dict) else []
        row = rows[0] if isinstance(rows, list) and rows and isinstance(rows[0], dict) else {}
        status = str(row.get("status", "")).lower()
        symbol_type = str(row.get("symbolType", row.get("innovation", ""))).lower()
        if status == "prelaunch":
            fee = cfg.fee_taker_bybit_premarket_pct
        elif symbol_type in {"innovation", "1", "true"}:
            fee = cfg.fee_taker_bybit_innovation_pct
        elif row:
            fee = cfg.fee_taker_bybit_pct
    except Exception:
        fee = fallback
    _BYBIT_FEE_CLASS_CACHE[symbol] = (now, max(0.0, float(fee)))
...
def _fetch_bybit_depth(symbol: str, cfg: Config) -> Optional[dict]:
    ...
    book = _mk_depth_book(result.get("b"), result.get("a"), venue="bybit-perp", ts=ts / 1000.0 if ts > 1e12 else time.time())
    if book is not None:
        book["taker_fee_pct"] = _bybit_taker_fee_for_symbol(symbol, cfg)
    return book
...
def _lighter_market_id_for_symbol(symbol: str) -> int:
    try:
        _q, stats = _LIGHTER_STREAM.snapshot()
        row = stats.get(symbol, {}) if isinstance(stats, dict) else {}
        mid = int(fnum(row.get("market_id", -1), default=-1))
        if mid >= 0:
            return mid
    except Exception:
        pass
    row = _LIGHTER_BOOTSTRAP_STATS.get(symbol, {}) if isinstance(_LIGHTER_BOOTSTRAP_STATS, dict) else {}
    return int(fnum(row.get("market_id", -1), default=-1))


def _fetch_lighter_depth_by_symbol(symbol: str, cfg: Config) -> Optional[dict]:
    market_id = _lighter_market_id_for_symbol(symbol)
    if market_id < 0:
        return None
    _LIGHTER_STREAM.subscribe_order_book(market_id)
    # External Lighter may be a newly-selected hedge venue, so unlike a Lighter
    # target it may not have been subscribed during Stage 1. Wait briefly for the
    # initial full snapshot instead of incorrectly treating a low-fee route as
    # unavailable. Still reject stale books for economics.
    deadline = time.time() + max(0.0, cfg.lighter_book_warmup_seconds)
    while True:
        book = _LIGHTER_STREAM.depth_snapshot(market_id, cfg.execution_book_levels)
        if book and time.time() - fnum(book.get("ts")) <= max(0.25, cfg.execution_max_book_age_seconds):
            return book
...
def _fetch_hyperliquid_depth_by_symbol(symbol: str, cfg: Config) -> Optional[dict]:
    coin = _HYPE_NATIVE.get(symbol) or (symbol[:-4] if symbol.endswith("USDT") else symbol)
    return _fetch_hyperliquid_depth(str(coin), cfg)


def _fetch_dydx_depth(symbol: str, cfg: Config) -> Optional[dict]:
    base = symbol[:-4] if symbol.endswith("USDT") else symbol
    ticker = f"{base}-USD"
    data = get_json(f"{DYDX_BASE}/orderbooks/perpetualMarket/{ticker}", cfg.request_timeout_seconds)
    if not isinstance(data, dict):
        return None
    return _mk_depth_book(data.get("bids"), data.get("asks"), venue="dydx-perp", ts=time.time())
...
def _fetch_external_execution_books(symbol: str, refs: List[str], cfg: Config) -> Dict[str, dict]:
    # V15 can execute against every venue for which we have a real depth API.
    # `refs` remains a hint, but we also add low-fee venues so a positive route
    # is not missed merely because that venue was midpoint-only in fair consensus.
    names = set(refs or [])
    names.update({"aster-perp", "bitget-perp", "mexc-perp", "bybit-perp", "hyperliquid-perp", "lighter-perp", "dydx-perp"})
    workers: List[Tuple[str, Callable[[], Optional[dict]]]] = []
    if "aster-perp" in names:
        workers.append(("aster-perp", lambda: _fetch_aster_depth(symbol, cfg)))
    ...
    if "hyperliquid-perp" in names:
        workers.append(("hyperliquid-perp", lambda: _fetch_hyperliquid_depth_by_symbol(symbol, cfg)))
    if "lighter-perp" in names:
        workers.append(("lighter-perp", lambda: _fetch_lighter_depth_by_symbol(symbol, cfg)))
    if "dydx-perp" in names:
        workers.append(("dydx-perp", lambda: _fetch_dydx_depth(symbol, cfg)))
    out: Dict[str, dict] = {}
    if not workers:
        return out
    with ThreadPoolExecutor(max_workers=min(7, len(workers))) as pool:
...
def execution_readiness_probe(c: Candidate, m: dict, side: str, cfg: Config) -> dict:
    """Fresh public-depth economics check. Never places orders.

    Positive economics means: current entry VWAP spread - conservative four-taker
    round-trip fees - an exit-slippage reserve > configured minimum. Funding is
    tracked separately because its sign and settlement timing are path-dependent.
    """
    ...
    target = str(getattr(c, "target", "aster"))
    ext_books.pop(f"{target}-perp", None)
    ...
    for usd in notionals:
        ...
        for venue, book in ext_books.items():
            # Do not use a second anomalous venue as the hedge just because it
            # creates a large synthetic pair spread. It must remain near fair.
            btop = book.get("bids", [])
            atop = book.get("asks", [])
            if not btop or not atop:
                continue
            book_mid = (float(btop[0][0]) + float(atop[0][0])) / 2.0
            if fair > 0 and abs(book_mid - fair) / fair * 100.0 > cfg.execution_max_hedge_fair_deviation_pct:
                continue
            ...
            target_fee = venue_taker_fee_pct(target, cfg)
            external_fee = fnum(book.get("taker_fee_pct"), venue_taker_fee_pct(venue, cfg))
            if target_fee >= 900 or external_fee >= 900:
                continue
            fees = 2.0 * (target_fee + external_fee)
            exit_slip_reserve = max(0.0, cfg.economic_exit_slippage_multiplier) * (
                max(0.0, fnum(tfill.get("slippage_pct"))) + max(0.0, fnum(efill.get("slippage_pct")))
            )
            net = gross - fees - exit_slip_reserve
            row = {
                "status": "READY" if net >= max(cfg.economic_min_net_edge_pct, cfg.execution_min_net_edge_pct) else "EDGE_TOO_THIN",
                "gross_edge_pct": gross, "net_edge_pct": net, "external_venue": venue,
                "roundtrip_fees_pct": fees,
                "target_taker_fee_pct": target_fee,
                "external_taker_fee_pct": external_fee,
                "exit_slippage_reserve_pct": exit_slip_reserve,
    ...
    return {
        ...
        "execution_roundtrip_fees_pct": (best_at_min or {}).get("roundtrip_fees_pct"),
        "execution_exit_slippage_reserve_pct": (best_at_min or {}).get("exit_slippage_reserve_pct"),
        "execution_target_taker_fee_pct": (best_at_min or {}).get("target_taker_fee_pct"),
        "execution_external_taker_fee_pct": (best_at_min or {}).get("external_taker_fee_pct"),
    }



# ------------------------------ V16 persistent paper ledger ------------------------------


def lag_route_profile_key(state_key: str, side: str, hedge_venue: str) -> str:
    """Stable key for exact-route LAG history.

    Route history is deliberately more specific than the older consensus
    profile: target:symbol + target side + the exact hedge venue actually used.
    """
    return f"{state_key}:{str(side).upper()}:{_paper_normalize_venue(hedge_venue)}"


def lag_route_profile_rows(
    state: dict, state_key: str, side: str, hedge_venue: str, cfg: Config,
    now: Optional[float] = None,
) -> List[dict]:
    now = time.time() if now is None else float(now)
    cutoff = now - max(1, cfg.active_lag_profile_lookback_hours) * 3600
    key = lag_route_profile_key(state_key, side, hedge_venue)
    rows = state.get("lag_route_profiles", {}).get(key, [])
    out: List[dict] = []
    for row in rows if isinstance(rows, list) else []:
        if not isinstance(row, dict) or fnum(row.get("ts")) < cutoff:
            continue
        out.append(row)
    return out[-cfg.active_lag_profile_max_points :]


def summarize_lag_route_profile(rows: List[dict]) -> dict:
    convs = [max(0.0, min(1.0, fnum(r.get("max_convergence_fraction")))) for r in rows]
    t50 = [fnum(r.get("time_to_50_seconds"), -1.0) for r in rows]
    t50 = [x for x in t50 if x >= 0]
    t80 = [fnum(r.get("time_to_80_seconds"), -1.0) for r in rows]
    t80 = [x for x in t80 if x >= 0]
    nets = [fnum(r.get("realized_net_pnl_pct_x")) for r in rows]
    wins = sum(1 for x in nets if x > 1e-12)
    return {
        "route_profile_episodes": len(rows),
        "route_median_max_convergence_fraction": statistics.median(convs) if convs else 0.0,
        "route_median_time_to_50_seconds": statistics.median(t50) if t50 else None,
        "route_median_time_to_80_seconds": statistics.median(t80) if t80 else None,
        "route_profitable_episodes": wins,
        "route_win_rate": (wins / len(rows)) if rows else None,
        "route_median_realized_net_pct_x": statistics.median(nets) if nets else None,
    }


def _record_lag_route_profile(state: dict, trade: dict, cfg: Config) -> None:
    """Persist one CLOSED exact-route LAG paper episode for future entry economics."""
    if str(trade.get("strategy", "")).upper() != "LAG":
        return
    state_key = str(trade.get("state_key") or "")
    side = str(trade.get("target_side") or "").upper()
    hedge = str(trade.get("hedge_venue") or "")
    if not state_key or side not in {"LONG", "SHORT"} or not hedge:
        return
    key = lag_route_profile_key(state_key, side, hedge)
    bucket = state.setdefault("lag_route_profiles", {})
    rows = bucket.setdefault(key, [])
    if not isinstance(rows, list):
        bucket[key] = rows = []
    trade_id = str(trade.get("id") or "")
    if trade_id and any(isinstance(r, dict) and str(r.get("trade_id")) == trade_id for r in rows):
        return
    row = {
        "ts": int(fnum(trade.get("closed_ts"), time.time())),
        "trade_id": trade_id,
        "state_key": state_key,
        ...
        "side": side,
        "hedge_venue": hedge,
        "entry_gap_pct": fnum(trade.get("entry_gap_pct")),
        "max_convergence_fraction": max(0.0, min(1.0, fnum(trade.get("max_convergence_fraction")))),
        "time_to_50_seconds": trade.get("time_to_50_seconds"),
        "time_to_80_seconds": trade.get("time_to_80_seconds"),
        "holding_seconds": fnum(trade.get("holding_seconds")),
        "realized_net_pnl_pct_x": fnum(trade.get("realized_net_pnl_pct_x")),
        "close_reason": trade.get("close_reason"),
    }
    rows.append(row)
    cutoff = fnum(row.get("ts"), time.time()) - max(1, cfg.active_lag_profile_lookback_hours) * 3600 * 2
    rows[:] = [r for r in rows if isinstance(r, dict) and fnum(r.get("ts")) >= cutoff]
    cap = max(2, cfg.active_lag_profile_max_points * 2)
    if len(rows) > cap:
        del rows[:-cap]


def _paper_min_execution_row(event: dict) -> Optional[dict]:
    checks = event.get("execution_checks", {}) if isinstance(event.get("execution_checks"), dict) else {}
    min_usd = fnum(event.get("execution_min_notional_usd"), 0.0)
    if min_usd <= 0:
        return None
    key = str(int(min_usd) if float(min_usd).is_integer() else min_usd)
    row = checks.get(key)
    return dict(row) if isinstance(row, dict) else None


def _paper_normalize_venue(venue: str) -> str:
...
def _paper_fetch_book_for_venue(venue: str, symbol: str, cfg: Config) -> Optional[dict]:
    v = _paper_normalize_venue(venue)
    if v == "aster":
        return _fetch_aster_depth(symbol, cfg)
    if v == "hyperliquid":
        return _fetch_hyperliquid_depth_by_symbol(symbol, cfg)
    if v == "lighter":
        return _fetch_lighter_depth_by_symbol(symbol, cfg)
    if v == "bitget":
        return _fetch_bitget_depth(symbol, cfg)
    if v == "mexc":
        return _fetch_mexc_depth(symbol, cfg)
    if v == "bybit":
        return _fetch_bybit_depth(symbol, cfg)
    if v == "dydx":
        return _fetch_dydx_depth(symbol, cfg)
    return None


def _paper_pair_books(trade: dict, cfg: Config) -> Tuple[Optional[dict], Optional[dict]]:
    symbol = str(trade.get("symbol") or "")
    target = str(trade.get("target") or "")
    hedge = str(trade.get("hedge_venue") or "")
    if not symbol or not target or not hedge:
        return None, None
    out: Dict[str, Optional[dict]] = {"target": None, "hedge": None}
    with ThreadPoolExecutor(max_workers=2) as pool:
        futs = {
            pool.submit(_paper_fetch_book_for_venue, target, symbol, cfg): "target",
            pool.submit(_paper_fetch_book_for_venue, hedge, symbol, cfg): "hedge",
        }
        for fut in as_completed(futs):
            try:
                ...
                out[futs[fut]] = None
    return out["target"], out["hedge"]


def _paper_used_capacity_by_venue(state: dict) -> Dict[str, float]:
    used: Dict[str, float] = {}
    ...
    for row in bucket.values() if isinstance(bucket, dict) else []:
        if not isinstance(row, dict) or str(row.get("status", "OPEN")) != "OPEN":
            continue
        alloc = max(0.0, fnum(row.get("capital_fraction_x"), 1.0))
        for venue in (row.get("target"), row.get("hedge_venue")):
            v = _paper_normalize_venue(str(venue or ""))
            if v:
                used[v] = used.get(v, 0.0) + alloc
    return used


def _paper_stats_bucket(container: dict, key: str) -> dict:
    return container.setdefault(key, {
        "opened": 0, "closed": 0, "wins": 0, "losses": 0, "breakeven": 0,
        "realized_net_pnl_pct_x": 0.0, "realized_gross_pnl_pct_x": 0.0,
        "fees_pct_x": 0.0, "funding_pct_x": 0.0, "realized_pnl_x": 0.0,
    })


def _paper_note_open(state: dict, trade: dict) -> None:
    stats = state.setdefault("paper_portfolio_stats", {})
    stats["signals_seen"] = int(fnum(stats.get("signals_seen"))) + 1
    stats["trades_opened"] = int(fnum(stats.get("trades_opened"))) + 1
    stats["last_open_ts"] = int(fnum(trade.get("opened_ts"), time.time()))
    for container, key in (
        (stats.setdefault("by_strategy", {}), str(trade.get("strategy", "UNKNOWN"))),
        (stats.setdefault("by_route", {}), f"{_paper_normalize_venue(trade.get('target'))}->{_paper_normalize_venue(trade.get('hedge_venue'))}"),
        (stats.setdefault("by_symbol", {}), str(trade.get("state_key") or trade.get("symbol") or "UNKNOWN")),
    ):
        b = _paper_stats_bucket(container, key)
        b["opened"] = int(fnum(b.get("opened"))) + 1


def _paper_note_skip(state: dict, event: dict, reason: str, cfg: Config) -> None:
    stats = state.setdefault("paper_portfolio_stats", {})
    stats["signals_seen"] = int(fnum(stats.get("signals_seen"))) + 1
    stats["skipped"] = int(fnum(stats.get("skipped"))) + 1
    if reason == "VENUE_CAPITAL_BUSY":
        stats["skipped_capital"] = int(fnum(stats.get("skipped_capital"))) + 1
    rows = state.setdefault("paper_signals_skipped", [])
    if isinstance(rows, list):
        rows.append({
            "ts": fnum(event.get("ts"), time.time()), "type": event.get("type"),
            "state_key": event.get("state_key"), "symbol": event.get("symbol"),
            "target": event.get("target"),
            "hedge_venue": event.get("execution_best_external_venue"),
            "side": event.get("side") or event.get("latest_action"),
            "net_edge_pct": event.get("execution_fresh_net_edge_pct"),
            "reason": reason,
        })
        cap = max(1, cfg.paper_max_skipped_signals)
        if len(rows) > cap:
            del rows[:-cap]


def open_paper_trade_from_signal(state: dict, event: dict, cfg: Config) -> bool:
    """Persist an actionable signal as a normalized paper trade.

    Only ACTIVE-LAG and ACTIVE-MM-EXCURSION are accepted. Broad spread-only
    POSITIVE-ECONOMICS observations are intentionally not paper entries in V16.
    """
    if not cfg.paper_trading_enabled or not isinstance(event, dict):
        return False
    typ = str(event.get("type", ""))
    if typ not in {"ACTIVE-LAG", "ACTIVE-MM-EXCURSION"}:
        return False
    strategy = "LAG" if typ == "ACTIVE-LAG" else "MM"
    if strategy == "MM" and cfg.paper_mm_require_live_excursion and bool(event.get("latest_reverted")):
        _paper_note_skip(state, event, "MM_ALREADY_REVERTED", cfg)
        return True
    if not bool(event.get("execution_ready")):
        return False
    fresh_net = fnum(event.get("execution_fresh_net_edge_pct"), -999.0)
    net = (
        fnum(event.get("convergence_adjusted_net_pct"), fresh_net)
        if strategy == "LAG" else fresh_net
    )
    if net <= cfg.paper_min_entry_net_edge_pct:
        return False
    side = str(event.get("side") or event.get("latest_action") or "").upper()
    if side not in {"LONG", "SHORT"}:
        return False
    target = _paper_normalize_venue(str(event.get("target") or "aster"))
    hedge = str(event.get("execution_best_external_venue") or event.get("preferred_hedge_venue") or "").lower()
    hedge_norm = _paper_normalize_venue(hedge)
    if not target or not hedge or hedge_norm == target:
        return False
    row = _paper_min_execution_row(event)
    if not row or str(row.get("status")) != "READY":
        return False
    entry_target = fnum(row.get("target_vwap"))
    entry_hedge = fnum(row.get("external_vwap"))
    if entry_target <= 0 or entry_hedge <= 0:
        return False

    bucket = state.setdefault("paper_trades_open", {})
    if not isinstance(bucket, dict):
        state["paper_trades_open"] = bucket = {}
    duplicate_key = f"{strategy}:{event.get('state_key')}:{side}:{hedge_norm}"
    for tr in bucket.values():
        if isinstance(tr, dict) and str(tr.get("status", "OPEN")) == "OPEN" and tr.get("dedupe_key") == duplicate_key:
            return False
    if len(bucket) >= max(1, cfg.paper_max_open_trades):
        _paper_note_skip(state, event, "OPEN_TRADE_CAP", cfg)
        return True

    alloc = max(0.0, min(cfg.paper_venue_capacity_x, cfg.paper_capital_fraction_per_trade))
    if alloc <= 0:
        return False
    if cfg.paper_enforce_venue_capital:
        used = _paper_used_capacity_by_venue(state)
        if used.get(target, 0.0) + alloc > cfg.paper_venue_capacity_x + 1e-12 or used.get(hedge_norm, 0.0) + alloc > cfg.paper_venue_capacity_x + 1e-12:
            _paper_note_skip(state, event, "VENUE_CAPITAL_BUSY", cfg)
            return True

    opened_ts = fnum(event.get("ts"), time.time())
    state_key = str(event.get("state_key") or f"{target}:{event.get('symbol')}")
    trade_id = f"paper:{strategy}:{state_key}:{side}:{hedge_norm}:{int(opened_ts*1000)}"
    if trade_id in bucket:
        return False
    target_fee = fnum(row.get("target_taker_fee_pct"), fnum(event.get("execution_target_taker_fee_pct"), venue_taker_fee_pct(target, cfg)))
    hedge_fee = fnum(row.get("external_taker_fee_pct"), fnum(event.get("execution_external_taker_fee_pct"), venue_taker_fee_pct(hedge, cfg)))
    probe_usd = fnum(event.get("execution_min_notional_usd"), cfg.paper_probe_notional_usd)
    entry_gap = max(0.0, fnum(row.get("gross_edge_pct"), fnum(event.get("execution_fresh_gross_edge_pct"))))
    trade = {
        "id": trade_id, "dedupe_key": duplicate_key, "strategy": strategy,
        "source_event_type": typ, "state_key": state_key, "symbol": event.get("symbol"),
        "target": target, "target_label": event.get("target_label") or TARGET_LABELS.get(target, target.title()),
        "hedge_venue": hedge, "target_side": side, "hedge_side": "LONG" if side == "SHORT" else "SHORT",
        "opened_ts": opened_ts, "status": "OPEN", "notional_x": max(0.0, cfg.paper_notional_x),
        "capital_fraction_x": alloc, "probe_notional_usd": probe_usd,
        "entry_target_vwap": entry_target, "entry_hedge_vwap": entry_hedge,
        "entry_gap_pct": entry_gap,
        "entry_estimated_net_edge_pct": fresh_net,
        "entry_expected_net_edge_pct": net,
        "entry_convergence_adjusted_net_pct": (event.get("convergence_adjusted_net_pct") if strategy == "LAG" else None),
        "entry_expected_capture_pct": (event.get("expected_capture_pct") if strategy == "LAG" else None),
        "entry_convergence_economics_fraction": (event.get("convergence_economics_fraction") if strategy == "LAG" else None),
        "entry_convergence_economics_source": (event.get("convergence_economics_source") if strategy == "LAG" else None),
        "entry_target_fee_pct": target_fee, "entry_hedge_fee_pct": hedge_fee,
        "entry_open_fees_pct_x": target_fee + hedge_fee,
        "expected_roundtrip_fees_pct_x": 2.0 * (target_fee + hedge_fee),
        "entry_exit_slippage_reserve_pct": fnum(row.get("exit_slippage_reserve_pct"), fnum(event.get("execution_exit_slippage_reserve_pct"))),
        "reference_disagreement_pct": fnum(event.get("reference_disagreement_pct")),
        "verified_episodes_at_entry": int(fnum(event.get("verified_episodes"))),
        "historical_median_convergence_fraction": (
            fnum(event.get("historical_median_convergence_fraction"), fnum(event.get("median_max_convergence_fraction")))
        ),
        "historical_median_t50_seconds": event.get("median_time_to_50_seconds"),
        "latest_same_side_gap_pct": entry_gap, "min_same_side_gap_pct": entry_gap,
        "max_same_side_gap_pct": entry_gap, "max_convergence_fraction": 0.0,
        "time_to_50_seconds": None, "time_to_80_seconds": None,
        "current_net_pnl_pct_x": -(target_fee + hedge_fee),
        "best_net_pnl_pct_x": -(target_fee + hedge_fee),
        "worst_net_pnl_pct_x": -(target_fee + hedge_fee),
        "funding_net_pnl_pct_x": 0.0, "updates": 0, "missing_depth_updates": 0,
        "checkpoints": {},
    }
    if strategy == "MM":
        trade.update({
            "mm_latest_direction": event.get("latest_direction"),
            "mm_signal_peak_pct": fnum(event.get("max_peak_pct"), fnum(event.get("median_peak_pct"))),
            "mm_regime_clean_reversion_rate": fnum(event.get("regime_clean_reversion_rate")),
            "mm_regime_median_reversion_seconds": event.get("regime_median_reversion_seconds"),
        })
    bucket[trade_id] = trade
    _paper_note_open(state, trade)
    return True


def _paper_funding_for_trade(trade: dict, cfg: Config, end_ts: float) -> Tuple[float, str]:
    if not cfg.funding_tracking_enabled:
        return 0.0, "DISABLED"
    start_ts = fnum(trade.get("opened_ts"))
    if end_ts <= start_ts:
        return 0.0, "READY"
    target_rows, target_err = _funding_rows_cached(str(trade.get("target")), str(trade.get("symbol")), start_ts, end_ts, cfg)
    hedge_rows, hedge_err = _funding_rows_cached(str(trade.get("hedge_venue")), str(trade.get("symbol")), start_ts, end_ts, cfg)
    target_rates = [fnum(x.get("rate_pct")) for x in target_rows if isinstance(x, dict)]
    hedge_rates = [fnum(x.get("rate_pct")) for x in hedge_rows if isinstance(x, dict)]
    target_pnl = _side_funding_pnl_pct(str(trade.get("target_side")), target_rates)
    hedge_pnl = _side_funding_pnl_pct(str(trade.get("hedge_side")), hedge_rates)
    return target_pnl + hedge_pnl, "READY" if not target_err and not hedge_err else "PARTIAL"


def _paper_mark_from_books(trade: dict, target_book: dict, hedge_book: dict, cfg: Config, now_ts: float) -> Optional[dict]:
    side = str(trade.get("target_side", "")).upper()
    usd = max(1e-9, fnum(trade.get("probe_notional_usd"), cfg.paper_probe_notional_usd))
    if side == "SHORT":
        tclose = _simulate_vwap(target_book, "BUY", usd)
        hclose = _simulate_vwap(hedge_book, "SELL", usd)
        tsame = _simulate_vwap(target_book, "SELL", usd)
        hsame = _simulate_vwap(hedge_book, "BUY", usd)
    elif side == "LONG":
        tclose = _simulate_vwap(target_book, "SELL", usd)
        hclose = _simulate_vwap(hedge_book, "BUY", usd)
        tsame = _simulate_vwap(target_book, "BUY", usd)
        hsame = _simulate_vwap(hedge_book, "SELL", usd)
    else:
        return None
    if not all(x.get("filled") for x in (tclose, hclose, tsame, hsame)):
        return None

    et = fnum(trade.get("entry_target_vwap")); eh = fnum(trade.get("entry_hedge_vwap"))
    if et <= 0 or eh <= 0:
        return None
    if side == "SHORT":
        target_leg = (et - fnum(tclose.get("vwap"))) / et * 100.0
        hedge_leg = (fnum(hclose.get("vwap")) - eh) / eh * 100.0
        current_gap = (fnum(tsame.get("vwap")) - fnum(hsame.get("vwap"))) / fnum(hsame.get("vwap")) * 100.0
    else:
        target_leg = (fnum(tclose.get("vwap")) - et) / et * 100.0
        hedge_leg = (eh - fnum(hclose.get("vwap"))) / eh * 100.0
        current_gap = (fnum(hsame.get("vwap")) - fnum(tsame.get("vwap"))) / fnum(hsame.get("vwap")) * 100.0
    target_exit_fee = fnum(target_book.get("taker_fee_pct"), venue_taker_fee_pct(str(trade.get("target")), cfg))
    hedge_exit_fee = fnum(hedge_book.get("taker_fee_pct"), venue_taker_fee_pct(str(trade.get("hedge_venue")), cfg))
    entry_fees = fnum(trade.get("entry_open_fees_pct_x"))
    exit_fees = target_exit_fee + hedge_exit_fee
    funding, funding_status = _paper_funding_for_trade(trade, cfg, now_ts)
    gross_pair = target_leg + hedge_leg
    net = gross_pair - entry_fees - exit_fees + funding
    entry_gap = max(1e-12, fnum(trade.get("entry_gap_pct")))
    convergence = (entry_gap - current_gap) / entry_gap
    return {
        "ts": now_ts, "target_close_vwap": fnum(tclose.get("vwap")), "hedge_close_vwap": fnum(hclose.get("vwap")),
        "current_same_side_gap_pct": current_gap, "convergence_fraction": convergence,
        "target_leg_pnl_pct_x": target_leg, "hedge_leg_pnl_pct_x": hedge_leg,
        "gross_pair_pnl_pct_x": gross_pair, "entry_fees_pct_x": entry_fees,
        "exit_fees_pct_x": exit_fees, "fees_total_pct_x": entry_fees + exit_fees,
        "funding_net_pnl_pct_x": funding, "funding_status": funding_status,
        "net_pnl_pct_x": net,
    }


def _paper_capture_checkpoints(trade: dict, mark: dict, cfg: Config) -> None:
    age_min = max(0.0, (fnum(mark.get("ts")) - fnum(trade.get("opened_ts"))) / 60.0)
    cps = trade.setdefault("checkpoints", {})
    for h in cfg.paper_checkpoint_minutes:
        label = _horizon_label(h)
        if label in cps or age_min + 1e-9 < h:
            continue
        cps[label] = {
            "ts": mark.get("ts"), "net_pnl_pct_x": mark.get("net_pnl_pct_x"),
            "gross_pair_pnl_pct_x": mark.get("gross_pair_pnl_pct_x"),
            "same_side_gap_pct": mark.get("current_same_side_gap_pct"),
            "convergence_fraction": mark.get("convergence_fraction"),
            "funding_net_pnl_pct_x": mark.get("funding_net_pnl_pct_x"),
        }


def _paper_close_reason(trade: dict, mark: dict, cfg: Config, now_ts: float) -> Optional[str]:
    strategy = str(trade.get("strategy", "LAG"))
    gap = fnum(mark.get("current_same_side_gap_pct"), 999.0)
    conv = fnum(mark.get("convergence_fraction"), -999.0)
    age = max(0.0, now_ts - fnum(trade.get("opened_ts")))
    if strategy == "MM":
        if gap <= cfg.paper_mm_close_edge_pct:
            return "MM_REVERSION_BAND"
        if conv >= cfg.paper_mm_close_convergence_fraction:
            return "MM_CONVERGENCE"
        if age >= max(1.0, cfg.paper_mm_max_hold_seconds):
            return "MM_TIMEOUT"
    else:
        if gap <= cfg.paper_lag_close_edge_pct:
            return "LAG_FULL_CONVERGENCE"
        if conv >= cfg.paper_lag_close_convergence_fraction:
            return "LAG_CONVERGENCE"
        if age >= max(60.0, cfg.paper_lag_max_hold_minutes * 60.0):
            return "LAG_TIMEOUT"
    return None


def _paper_note_close(state: dict, trade: dict, cfg: Config) -> None:
    stats = state.setdefault("paper_portfolio_stats", {})
    stats["trades_closed"] = int(fnum(stats.get("trades_closed"))) + 1
    net = fnum(trade.get("realized_net_pnl_pct_x"))
    if net > 1e-12:
        stats["wins"] = int(fnum(stats.get("wins"))) + 1
    elif net < -1e-12:
        stats["losses"] = int(fnum(stats.get("losses"))) + 1
    else:
        stats["breakeven"] = int(fnum(stats.get("breakeven"))) + 1
    stats["realized_net_pnl_pct_x"] = fnum(stats.get("realized_net_pnl_pct_x")) + net
    stats["realized_pnl_x"] = fnum(stats.get("realized_pnl_x")) + net / 100.0 * fnum(trade.get("notional_x"), 1.0) * fnum(trade.get("capital_fraction_x"), 1.0)
    stats["last_close_ts"] = int(fnum(trade.get("closed_ts"), time.time()))
    for container, key in (
        (stats.setdefault("by_strategy", {}), str(trade.get("strategy", "UNKNOWN"))),
        (stats.setdefault("by_route", {}), f"{_paper_normalize_venue(trade.get('target'))}->{_paper_normalize_venue(trade.get('hedge_venue'))}"),
        (stats.setdefault("by_symbol", {}), str(trade.get("state_key") or trade.get("symbol") or "UNKNOWN")),
        ...
        b = _paper_stats_bucket(container, key)
        b["closed"] = int(fnum(b.get("closed"))) + 1
        if net > 1e-12: b["wins"] = int(fnum(b.get("wins"))) + 1
        elif net < -1e-12: b["losses"] = int(fnum(b.get("losses"))) + 1
        else: b["breakeven"] = int(fnum(b.get("breakeven"))) + 1
        b["realized_net_pnl_pct_x"] = fnum(b.get("realized_net_pnl_pct_x")) + net
        b["realized_gross_pnl_pct_x"] = fnum(b.get("realized_gross_pnl_pct_x")) + fnum(trade.get("realized_gross_pnl_pct_x"))
        b["fees_pct_x"] = fnum(b.get("fees_pct_x")) + fnum(trade.get("realized_fees_pct_x"))
        b["funding_pct_x"] = fnum(b.get("funding_pct_x")) + fnum(trade.get("realized_funding_pct_x"))
        b["realized_pnl_x"] = fnum(b.get("realized_pnl_x")) + net / 100.0 * fnum(trade.get("notional_x"), 1.0) * fnum(trade.get("capital_fraction_x"), 1.0)
    _record_lag_route_profile(state, trade, cfg)


def update_paper_trades(state: dict, cfg: Config, now_ts: Optional[float] = None) -> bool:
    """Mark every OPEN paper trade using the exact stored target+hedge route.

    V16.1 fetches route books concurrently. The old implementation refreshed
    positions sequentially; as persistent paper positions accumulated, two REST
    book requests per trade could make a GitHub Actions run take many minutes
    and create a permanent concurrency queue.
    """
    ...
    open_rows = [
        (tid, trade) for tid, trade in list(bucket.items())
        if isinstance(trade, dict) and str(trade.get("status", "OPEN")) == "OPEN"
    ]
    if not open_rows:
        return False

    # Network I/O dominates this phase, so a small bounded thread pool gives a
    # large speed-up without changing accounting semantics. Each trade is still
    # marked only against the exact target+hedge route stored at entry.
    workers = max(1, min(int(getattr(cfg, "paper_mark_workers", 8)), len(open_rows), 12))
    books_by_tid: Dict[str, Tuple[Optional[dict], Optional[dict]]] = {}
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(_paper_pair_books, trade, cfg): tid for tid, trade in open_rows}
        for fut in as_completed(futures):
            tid = futures[fut]
            try:
                books_by_tid[tid] = fut.result()
            except Exception:
                books_by_tid[tid] = (None, None)

    changed = False
    for tid, trade in open_rows:
        # A trade could only disappear here if future code mutates the bucket
        # concurrently; keep this defensive check so state accounting is safe.
        if tid not in bucket:
            continue
        target_book, hedge_book = books_by_tid.get(tid, (None, None))
        if not target_book or not hedge_book:
            trade["missing_depth_updates"] = int(fnum(trade.get("missing_depth_updates"))) + 1
            trade["last_update_ts"] = now_ts
            changed = True
            continue
        mark = _paper_mark_from_books(trade, target_book, hedge_book, cfg, now_ts)
        if not mark:
            trade["missing_depth_updates"] = int(fnum(trade.get("missing_depth_updates"))) + 1
            trade["last_update_ts"] = now_ts
            changed = True
            continue
        trade["updates"] = int(fnum(trade.get("updates"))) + 1
        trade["last_update_ts"] = now_ts
        trade["latest_same_side_gap_pct"] = mark["current_same_side_gap_pct"]
        trade["min_same_side_gap_pct"] = min(fnum(trade.get("min_same_side_gap_pct"), mark["current_same_side_gap_pct"]), mark["current_same_side_gap_pct"])
        trade["max_same_side_gap_pct"] = max(fnum(trade.get("max_same_side_gap_pct"), mark["current_same_side_gap_pct"]), mark["current_same_side_gap_pct"])
        trade["max_convergence_fraction"] = max(fnum(trade.get("max_convergence_fraction")), mark["convergence_fraction"])
        age_seconds = max(0.0, now_ts - fnum(trade.get("opened_ts")))
        if mark["convergence_fraction"] >= 0.50 and trade.get("time_to_50_seconds") is None:
            trade["time_to_50_seconds"] = age_seconds
        if mark["convergence_fraction"] >= 0.80 and trade.get("time_to_80_seconds") is None:
            trade["time_to_80_seconds"] = age_seconds
        trade["current_net_pnl_pct_x"] = mark["net_pnl_pct_x"]
        trade["best_net_pnl_pct_x"] = max(fnum(trade.get("best_net_pnl_pct_x"), mark["net_pnl_pct_x"]), mark["net_pnl_pct_x"])
        trade["worst_net_pnl_pct_x"] = min(fnum(trade.get("worst_net_pnl_pct_x"), mark["net_pnl_pct_x"]), mark["net_pnl_pct_x"])
        trade["funding_net_pnl_pct_x"] = mark["funding_net_pnl_pct_x"]
        trade["funding_status"] = mark["funding_status"]
        trade["last_target_close_vwap"] = mark["target_close_vwap"]
        trade["last_hedge_close_vwap"] = mark["hedge_close_vwap"]
        _paper_capture_checkpoints(trade, mark, cfg)
        reason = _paper_close_reason(trade, mark, cfg, now_ts)
        if reason:
            trade["status"] = "CLOSED"
            trade["closed_ts"] = now_ts
            trade["close_reason"] = reason
            trade["exit_target_vwap"] = mark["target_close_vwap"]
            trade["exit_hedge_vwap"] = mark["hedge_close_vwap"]
            trade["exit_same_side_gap_pct"] = mark["current_same_side_gap_pct"]
            trade["realized_target_leg_pnl_pct_x"] = mark["target_leg_pnl_pct_x"]
            trade["realized_hedge_leg_pnl_pct_x"] = mark["hedge_leg_pnl_pct_x"]
            trade["realized_gross_pnl_pct_x"] = mark["gross_pair_pnl_pct_x"]
            trade["realized_fees_pct_x"] = mark["fees_total_pct_x"]
            trade["realized_funding_pct_x"] = mark["funding_net_pnl_pct_x"]
            trade["realized_net_pnl_pct_x"] = mark["net_pnl_pct_x"]
            trade["holding_seconds"] = max(0.0, now_ts - fnum(trade.get("opened_ts")))
            trade["realized_pnl_x"] = mark["net_pnl_pct_x"] / 100.0 * fnum(trade.get("notional_x"), 1.0) * fnum(trade.get("capital_fraction_x"), 1.0)
            _paper_note_close(state, trade, cfg)
            closed.append(dict(trade))
            bucket.pop(tid, None)
        changed = True
    cap = max(1, cfg.paper_max_closed_trades)
    if len(closed) > cap:
        del closed[:-cap]
        changed = True
    return changed
...
def _update_mm_profile_from_outcome(state: dict, row: dict, cfg: Config) -> None:
    skey = str(row.get("state_key") or "")
    if not skey:
        return
    profiles = state.setdefault("mm_profiles", {})
    p = profiles.setdefault(skey, {
        "observed_excursions": 0, "clean_reversions": 0, "reverted_not_clean": 0,
        "unresolved": 0, "signal_sent": 0, "sum_peak_pct": 0.0, "max_peak_pct": 0.0,
        "peak_pct_samples": [], "clean_reversion_seconds_samples": [],
        "last_excursion_ts": 0, "last_clean_reversion_ts": 0,
    })
    p["observed_excursions"] = int(fnum(p.get("observed_excursions"))) + 1
    status = str(row.get("status", ""))
    if status == "CLEAN_REVERTED":
        p["clean_reversions"] = int(fnum(p.get("clean_reversions"))) + 1
        rev = fnum(row.get("reversion_seconds"), -1.0)
        if rev >= 0:
            p.setdefault("clean_reversion_seconds_samples", []).append(rev)
        p["last_clean_reversion_ts"] = max(fnum(p.get("last_clean_reversion_ts")), fnum(row.get("closed_ts")))
    elif status == "REVERTED_NOT_CLEAN":
        p["reverted_not_clean"] = int(fnum(p.get("reverted_not_clean"))) + 1
    else:
        p["unresolved"] = int(fnum(p.get("unresolved"))) + 1
    if row.get("signal_sent"):
        p["signal_sent"] = int(fnum(p.get("signal_sent"))) + 1
    peak = fnum(row.get("max_peak_pct"), fnum(row.get("signal_peak_pct")))
    p["sum_peak_pct"] = fnum(p.get("sum_peak_pct")) + peak
    p["max_peak_pct"] = max(fnum(p.get("max_peak_pct")), peak)
    p.setdefault("peak_pct_samples", []).append(peak)
    p["last_excursion_ts"] = max(fnum(p.get("last_excursion_ts")), fnum(row.get("start_ts")))
    cap = max(5, cfg.paper_mm_profile_sample_cap)
    for key in ("peak_pct_samples", "clean_reversion_seconds_samples"):
        vals = p.get(key, [])
        if isinstance(vals, list) and len(vals) > cap:
            del vals[:-cap]
    n = max(1, int(fnum(p.get("observed_excursions"))))
    p["clean_reversion_rate"] = int(fnum(p.get("clean_reversions"))) / n
    peaks = p.get("peak_pct_samples", []) if isinstance(p.get("peak_pct_samples"), list) else []
    revs = p.get("clean_reversion_seconds_samples", []) if isinstance(p.get("clean_reversion_seconds_samples"), list) else []
    p["median_peak_pct_recent"] = statistics.median(peaks) if peaks else None
    p["median_clean_reversion_seconds_recent"] = statistics.median(revs) if revs else None


def _lag_convergence_adjusted_economics(event: dict, cfg: Config) -> dict:
    """Return convergence-adjusted economics for one ACTIVE-LAG event.

    Fresh execution economics assumes the entire current cross-venue spread can
    eventually be captured. For an entry decision we instead multiply the fresh
    executable spread by the median convergence fraction observed on the exact
    target/symbol/side/hedge route whenever closed route history exists. Until
    route history is available, the older consensus convergence profile is used
    only as a bootstrap; same-run convergence is the final fallback. Costs remain the full round-trip taker fees plus exit-slippage
    reserve because those costs are paid regardless of how much of the spread
    ultimately converges.
    """
    route_episodes = int(fnum(event.get("route_profile_episodes")))
    route_raw = event.get("route_median_max_convergence_fraction")
    route = fnum(route_raw, -1.0) if route_raw is not None else -1.0
    hist_raw = event.get("historical_median_convergence_fraction")
    same_raw = event.get("same_run_convergence_fraction")
    hist = fnum(hist_raw, -1.0) if hist_raw is not None else -1.0
    same = fnum(same_raw, -1.0) if same_raw is not None else -1.0
    fallback = fnum(event.get("median_max_convergence_fraction"), 0.0)

    if route_episodes >= max(1, cfg.active_lag_min_route_profile_episodes) and route >= 0.0:
        fraction = route
        source = "exact-route-history"
    elif hist > 0.0:
        fraction = hist
        source = "historical-consensus-bootstrap"
    elif same > 0.0:
        fraction = same
        source = "same-run-bootstrap"
    else:
        fraction = fallback
        source = "profile-fallback"

    fraction = max(0.0, min(1.0, fraction))
    fresh_gross = max(0.0, fnum(event.get("execution_fresh_gross_edge_pct"), fnum(event.get("gross_edge_pct"))))
    fees = max(0.0, fnum(event.get("execution_roundtrip_fees_pct")))
    exit_slip = max(0.0, fnum(event.get("execution_exit_slippage_reserve_pct")))
    expected_capture = fresh_gross * fraction
    adjusted_net = expected_capture - fees - exit_slip
    return {
        "convergence_economics_fraction": fraction,
        "convergence_economics_source": source,
        "expected_capture_pct": expected_capture,
        "convergence_adjusted_net_pct": adjusted_net,
        "convergence_adjusted_cost_pct": fees + exit_slip,
    }



def _reopen_active_lag_latch(state: Optional[dict], c: Candidate, side: str) -> None:
    if state is None:
        return
    latch = f"{candidate_state_key(c)}:{side}"
    bucket = state.setdefault("active_signal_open", {}).get("ACTIVE-LAG", [])
    if isinstance(bucket, list) and latch in bucket:
        bucket.remove(latch)


def _fresh_lag_baseline_ok(event: dict, cfg: Config) -> bool:
    """Re-apply READY baseline gating to the fresh exact execution route.

    Detection happens before the depth probe and can be based on consensus BBO.
    The fresh best route may have a smaller executable edge. If the baseline is
    READY, the actual route we would trade must itself clear the anomaly level.
    """
    points = int(fnum(event.get("baseline_points")))
    required = event.get("baseline_required_gap_pct")
    if points < cfg.lag_baseline_min_points or required is None:
        return True
    fresh = fnum(event.get("execution_fresh_gross_edge_pct"), fnum(event.get("gross_edge_pct"), -999.0))
    return fresh >= fnum(required, 999.0)


def _attach_exact_route_history(event: dict, state: Optional[dict], cfg: Config) -> None:
    if state is None:
        return
    hedge = str(event.get("preferred_hedge_venue") or event.get("execution_best_external_venue") or "")
    state_key = str(event.get("state_key") or "")
    side = str(event.get("side") or "").upper()
    if not state_key or side not in {"LONG", "SHORT"} or not hedge:
        return
    rows = lag_route_profile_rows(state, state_key, side, hedge, cfg)
    event.update(summarize_lag_route_profile(rows))
    event["route_profile_key"] = lag_route_profile_key(state_key, side, hedge)
...
def _attach_execution_probe_to_events(events: List[dict], candidates: List[Candidate], cfg: Config, state: Optional[dict] = None) -> List[dict]:
    """Attach fresh venue-specific economics and suppress non-positive entry signals.

    ACTIVE-LAG is now a Telegram entry-opportunity signal only if the requested
    minimum notional is fully executable and remains positive after conservative
    open+close taker fees plus exit-slippage reserve. Rejected latches are
    reopened so a structurally active spread can alert later if economics improve.
    """
    by_key = {candidate_state_key(c): c for c in candidates}
    accepted: List[dict] = []
    for ev in events:
        c = by_key.get(str(ev.get("state_key", "")))
        side = str(ev.get("side", "NONE")).upper()
        if not c or side not in {"LONG", "SHORT"}:
            continue
        m = c.metrics(cfg)
        if fnum(ev.get("bbo_coverage_ratio")) <= 0:
            ev["bbo_coverage_ratio"] = _candidate_bbo_coverage(c)
        try:
            ev.update(cached_execution_probe(c, m, side, cfg, force_fresh=True))
        except Exception as e:
        ...
        if not ev.get("execution_ready"):
            # Underlying research detector can be active while trading economics
            # are not. Remove only this newly-created latch so the next run can
            # re-probe and alert once the route becomes genuinely positive.
            _reopen_active_lag_latch(state, c, side)
            c.active_lag_detected = False
            continue

        # V16.4: if the structural baseline is READY, re-check it against the
        # exact fresh route selected by the depth probe. This prevents a smaller
        # tradeable route from alerting merely because the pre-probe consensus
        # edge happened to clear the anomaly threshold.
        ev["fresh_route_baseline_anomalous"] = _fresh_lag_baseline_ok(ev, cfg)
        if not ev["fresh_route_baseline_anomalous"]:
            _reopen_active_lag_latch(state, c, side)
            c.active_lag_detected = False
            continue

        # Prefer exact-route paper history once available. Consensus convergence
        # remains only a bootstrap for routes that have never closed a paper trade.
        _attach_exact_route_history(ev, state, cfg)
        ev.update(_lag_convergence_adjusted_economics(ev, cfg))
        if fnum(ev.get("convergence_adjusted_net_pct"), -999.0) <= cfg.active_lag_min_convergence_adjusted_net_pct:
            _reopen_active_lag_latch(state, c, side)
            c.active_lag_detected = False
            continue

        # Fresh route prices remain authoritative for the current executable
        # gross/full-capture net. The convergence-adjusted value is stored
        # separately and is the economic expectation used for LAG entry quality.
        ev["gross_edge_pct"] = fnum(ev.get("execution_fresh_gross_edge_pct"), fnum(ev.get("gross_edge_pct")))
        ev["net_edge_pct"] = fnum(ev.get("execution_fresh_net_edge_pct"), fnum(ev.get("net_edge_pct")))
        accepted.append(ev)
        if state is not None:
            try:
                skey = str(ev.get("state_key", candidate_state_key(c)))
                record_signal_stats(state, "ACTIVE-LAG", skey, 1, {side: 1})
                record_signal_stats(state, "ACTIVE-LAG-READY", skey, 1, {side: 1})
                # Same-run detector used to start the outcome before economics;
                # start it only after a positive route is confirmed.
                if str(ev.get("trigger", "")).startswith("same-run"):
                    start_active_lag_outcome(state, ev, cfg)
            except Exception as e:
                print(f"WARN ACTIVE-LAG accounting: {type(e).__name__}: {e}", file=sys.stderr)
...
def detect_same_run_active_lag_events(candidates: List[Candidate], cfg: Config, state: dict) -> Tuple[List[dict], bool]:
    ...
    for c in candidates:
        ...
        if current < cfg.active_lag_min_gross_edge_pct: continue
        ...
        ev={"type":"ACTIVE-LAG","symbol":c.symbol,"state_key":skey,"target":getattr(c,"target","aster"),"target_label":target_label(c),
            ...
            "historical_median_convergence_fraction": (float(profile.get("median_max_convergence_fraction",0)) if int(profile.get("verified_episodes",0)) > 0 else None),
            "same_run_convergence_fraction": float(m.get("lag_max_convergence_fraction",0)),
...
def format_outcome_signal(event: dict, state: dict) -> str:
    ...
    if typ=="ACTIVE-LAG-CHECKPOINT":
        ...
        return (
            ...
            f"Estimated pair PnL if closed now: <b>{fnum(cp.get('estimated_pair_pnl_if_closed_pct')):+.3f}%</b> after entry cost reserve + settled funding\n"
    ...
    if typ=="ACTIVE-LAG-OUTCOME":
        ...
        return (
            ...
            f"{_fmt_funding_line(event)}\n"
            f"Estimated pair PnL at final observation: <b>{fnum(event.get('estimated_pair_pnl_if_closed_pct')):+.3f}%</b> after entry cost reserve + settled funding"
            f"{hs_block}\n"
...
def detect_positive_economics_events(candidates: List[Candidate], cfg: Config, state: dict) -> Tuple[List[dict], bool]:
    """Emit every newly-positive fresh execution route at the configured notional.

    No convergence-history requirement is imposed here; this is the broad signal
    layer requested for discovery. A transition latch prevents duplicate alerts
    while the same route remains continuously positive.
    """
    if not (cfg.economic_signals_enabled and cfg.execution_readiness_enabled):
        return [], False
    open_state = state.setdefault("active_signal_open", {})
    prev = set(open_state.get("POSITIVE-ECONOMICS", []) if isinstance(open_state.get("POSITIVE-ECONOMICS", []), list) else [])
    current: set[str] = set()
    rows: List[Tuple[float, Candidate, str, dict]] = []
    for c in candidates:
        if not c.samples:
            continue
        m = c.metrics(cfg)
        if float(m.get("ref_good_ratio", 0.0)) < cfg.min_ref_good_ratio:
            continue
        smp = c.samples[-1]
        potentials = {
            "SHORT": max(fnum(smp.hedge_short_edge_pct, -999), fnum(smp.short_edge_pct, -999), fnum(smp.best_hedge_short_edge_pct, -999)),
            "LONG": max(fnum(smp.hedge_long_edge_pct, -999), fnum(smp.long_edge_pct, -999), fnum(smp.best_hedge_long_edge_pct, -999)),
        }
        refs = list(dict.fromkeys(list(m.get("current_exec_refs", [])) + list(m.get("current_refs", []))))
        min_fee = minimum_possible_roundtrip_fee_pct(str(getattr(c, "target", "aster")), _DEPTH_EXEC_VENUES, cfg)
        required_gross = max(cfg.economic_probe_min_gross_pct, min_fee + cfg.economic_min_net_edge_pct)
        for side, potential in potentials.items():
            # Probe every side that could possibly remain positive under at least
            # one supported hedge route. The fresh depth probe is still the gate.
            if math.isfinite(potential) and math.isfinite(required_gross) and potential >= required_gross:
                rows.append((potential, c, side, m))
    rows.sort(key=lambda x: x[0], reverse=True)
    rows = rows[: max(1, cfg.economic_probe_max_candidates)]
    events: List[dict] = []
    now = time.time()
    cooldown = max(0, cfg.economic_signal_cooldown_minutes) * 60
    last_map = state.setdefault("economic_signal_last_ts", {})
    for _potential, c, side, m in rows:
        try:
            probe = cached_execution_probe(c, m, side, cfg, force_fresh=True)
        except Exception:
            continue
        if not probe.get("execution_ready"):
            continue
        venue = str(probe.get("execution_best_external_venue") or "")
        if not venue:
            continue
        latch = f"{candidate_state_key(c)}:{side}:{venue}"
        current.add(latch)
        last_ts = fnum(last_map.get(latch))
        if latch in prev and (cooldown <= 0 or now - last_ts < cooldown):
            continue
        ev = {
            "type": "POSITIVE-ECONOMICS",
            "symbol": c.symbol, "state_key": candidate_state_key(c),
            "target": getattr(c, "target", "aster"), "target_label": target_label(c),
            "ts": now, "side": side,
            "reference_disagreement_pct": float(m.get("current_ref_disagreement_pct", 0.0)),
            "persistent_hit_ratio": float(m.get("persistent_exec_hit_ratio", 0.0)),
        ...
        ev["gross_edge_pct"] = fnum(probe.get("execution_fresh_gross_edge_pct"))
        ev["net_edge_pct"] = fnum(probe.get("execution_fresh_net_edge_pct"))
        events.append(ev)
        last_map[latch] = int(now)
        record_signal_stats(state, "POSITIVE-ECONOMICS", candidate_state_key(c), 1, {side: 1})
    open_state["POSITIVE-ECONOMICS"] = sorted(current)
    return events, bool(events or prev != current)
...
def scan(
    ...
    # V14: OPEN ACTIVE-LAG outcomes are sampled even if the symbol no longer
    ...
    economic_events, economic_changed = detect_positive_economics_events(candidates, cfg, state)
    if active_signal_callback is not None:
    ...
    for c in candidates:
        ...
        if m:
            ranked.append((c, m))
    ranked.sort(key=lambda x: x[1]["score"], reverse=True)
    return ranked, errors, bool(migrated or tracking_changed or economic_changed or active_changed or extended_changed or same_run_changed or lag_outcome_changed or mm_outcome_changed)
...
def send_telegram_photo(path: Path, candidate: Candidate, m: dict) -> bool:
    ...
    if "MM" in m.get("setup", ""):
        caption = (
            ...
            "V15 MM statistics use actual target-venue trades matched to the time-local robust PERP consensus; user-facing MM signals additionally require positive fresh execution economics."
        )
    else:
        caption = (
            f"📈 <b>{candidate.symbol} @ {label} — {m['setup']}</b>\n"
...
def format_alert(candidate: Candidate, m: dict) -> str:
    ...
    econ = m.get("_economic_probe") if isinstance(m.get("_economic_probe"), dict) else {}
    econ_side = str(m.get("_economic_side") or persistent or side_text)
    econ_block = ""
    if econ:
        econ_block = (
            f"Fresh executable economics: <b>{econ_side}</b> target / hedge via <b>{econ.get('execution_best_external_venue') or 'unknown'}</b>\n"
            f"VWAP gross {fnum(econ.get('execution_fresh_gross_edge_pct')):.3f}% | four-taker fees {fnum(econ.get('execution_roundtrip_fees_pct')):.3f}% | exit-slip reserve {fnum(econ.get('execution_exit_slippage_reserve_pct')):.3f}% | net <b>{fnum(econ.get('execution_fresh_net_edge_pct')):+.3f}% ✅</b>\n"
            f"Execution ready: <b>${fnum(econ.get('execution_ready_notional_usd')):,.0f}</b>+ | minimum ${fnum(econ.get('execution_min_notional_usd')):,.0f}\n"
        )
    return (
        ...
        f"Consensus-BBO gross edge now: <b>{m['current_executable_edge_pct']:.3f}% {side_text}</b> (research view; profitability uses the fresh venue route below)\n"
        f"{econ_block}"
        ...
        f"Persistent side: <b>{persistent}</b> | gross-hit {m['persistent_exec_hit_ratio']*100:.0f}% | median gross {m['persistent_median_executable_edge_pct']:.3f}%\n"
        ...
        f"State: <b>{m['direction']}</b>\n\n"
        "V15 sends entry-style alerts only when fresh venue-specific execution economics are positive at the configured minimum notional. Signal only; no orders are sent."
    )
...
def format_active_signal(event: dict, state: dict) -> str:
    ...
    if typ == "POSITIVE-ECONOMICS":
        min_usd = fnum(event.get("execution_min_notional_usd"), default=100.0)
        return (
            f"💰 <b>POSITIVE ECONOMICS — {symbol} @ {label}</b>\n"
            f"Pair: <b>{event.get('side','NONE')} target</b> / hedge via <b>{event.get('execution_best_external_venue') or 'unknown'}</b>\n"
            f"Fresh VWAP @{min_usd:,.0f}: gross <b>{fnum(event.get('execution_fresh_gross_edge_pct')):.3f}%</b>\n"
            f"Round-trip taker fees: {fnum(event.get('execution_roundtrip_fees_pct')):.3f}% "
            f"(target {fnum(event.get('execution_target_taker_fee_pct')):.3f}% + hedge {fnum(event.get('execution_external_taker_fee_pct')):.3f}% each way)\n"
            f"Exit-slippage reserve: {fnum(event.get('execution_exit_slippage_reserve_pct')):.3f}%\n"
            f"Estimated net after costs: <b>{fnum(event.get('execution_fresh_net_edge_pct')):+.3f}%</b> ✅\n"
            f"Ready notional up to: <b>${fnum(event.get('execution_ready_notional_usd')):,.0f}</b> | ref disagreement {fnum(event.get('reference_disagreement_pct')):.3f}%\n"
            f"Gross persistence: {fnum(event.get('persistent_hit_ratio'))*100:.0f}% | median gross {fnum(event.get('persistent_median_edge_pct')):.3f}%\n\n"
            "Positive economics is a conservative snapshot estimate, not a guaranteed realized PnL; funding and future close-book changes remain variable. No orders are sent."
        )
    ...
    if typ == "ACTIVE-LAG":
        ...
        fresh_txt = "n/a" if fresh_gross is None else f"{float(fresh_gross):.3f}% gross / {float(fresh_net):+.3f}% full-capture net"
        conv_frac = fnum(event.get("convergence_economics_fraction"))
        expected_capture = fnum(event.get("expected_capture_pct"))
        adjusted_net = fnum(event.get("convergence_adjusted_net_pct"))
        conv_source = str(event.get("convergence_economics_source") or "historical-median")
        return (
            f"⚡ <b>ACTIVE-LAG — {symbol} @ {label}</b>\n"
            f"Side: <b>{side}</b> | pair interpretation: {pair}\n"
            f"Current executable gap: <b>{float(event.get('gross_edge_pct',0)):.3f}%</b> | full-capture net <b>{float(event.get('net_edge_pct',0)):+.3f}%</b>\n"
            f"Expected capture: {float(event.get('gross_edge_pct',0)):.3f}% × {conv_frac*100:.0f}% = <b>{expected_capture:.3f}%</b> [{conv_source}]\n"
            + (f"Exact route history: <b>{int(event.get('route_profile_episodes',0))}</b> closed | median convergence {fnum(event.get('route_median_max_convergence_fraction'))*100:.0f}% | win rate {fnum(event.get('route_win_rate'))*100:.0f}%\n" if int(fnum(event.get('route_profile_episodes'))) > 0 else "Exact route history: bootstrap from consensus; no closed route paper trades yet\n")
            + f"Convergence-adjusted net: <b>{adjusted_net:+.3f}% ✅</b> after fees + exit-slip reserve\n"
            ...
            f"V16.4 counter: {skey} detections <b>{per_target}</b> | all targets <b>{total}</b>\n\nSignal only. No API orders are sent."
        )
    if typ == "ACTIVE-MM-EXCURSION":
        ...
        return (
            ...
            f"Fresh pair economics: gross {fnum(event.get('execution_fresh_gross_edge_pct')):.3f}% | fees {fnum(event.get('execution_roundtrip_fees_pct')):.3f}% | exit-slip reserve {fnum(event.get('execution_exit_slippage_reserve_pct')):.3f}% | net <b>{fnum(event.get('execution_fresh_net_edge_pct')):+.3f}%</b> ✅\n"
            ...
            f"V16.4 counter: {skey} ACTIVE-MM events <b>{per_target}</b> | all targets <b>{total}</b>\n\nEvents are batched; no orders are sent."
        )
...
def _report_median(values: List[float]) -> Optional[float]:
    vals = [float(x) for x in values if x is not None and math.isfinite(float(x))]
    return statistics.median(vals) if vals else None


def _paper_report_strategy_summary(rows: List[dict], strategy: str) -> dict:
    rows = [r for r in rows if isinstance(r, dict) and str(r.get("strategy", "")).upper() == strategy]
    nets = [fnum(r.get("realized_net_pnl_pct_x")) for r in rows]
    wins = sum(1 for x in nets if x > 1e-12)
    holds = [fnum(r.get("holding_seconds")) for r in rows if r.get("holding_seconds") is not None]
    actual_conv = [max(0.0, min(1.0, fnum(r.get("max_convergence_fraction")))) for r in rows]
    if strategy == "LAG":
        predicted_conv = [
            max(0.0, min(1.0, fnum(r.get("entry_convergence_economics_fraction"))))
            for r in rows if r.get("entry_convergence_economics_fraction") is not None
        ]
        predicted_net = [
            fnum(r.get("entry_convergence_adjusted_net_pct"))
            for r in rows if r.get("entry_convergence_adjusted_net_pct") is not None
        ]
    else:
        predicted_conv = []
        predicted_net = [
            fnum(r.get("entry_expected_net_edge_pct"))
            for r in rows if r.get("entry_expected_net_edge_pct") is not None
        ]
    return {
        "strategy": strategy,
        "trades": len(rows),
        ...
        "median_predicted_convergence_fraction": _report_median(predicted_conv),
        "median_actual_max_convergence_fraction": _report_median(actual_conv),
        "median_predicted_net_pct_x": _report_median(predicted_net),
        "median_realized_net_pct_x": _report_median(nets),
        "mean_realized_net_pct_x": (sum(nets) / len(nets)) if nets else None,
        "median_hold_seconds": _report_median(holds),
    }


def _paper_report_route_rows(closed_rows: List[dict], top_n: int) -> List[dict]:
    groups: Dict[str, List[dict]] = {}
    for tr in closed_rows:
        if not isinstance(tr, dict):
            continue
        strategy = str(tr.get("strategy", "UNKNOWN")).upper()
        target = _paper_normalize_venue(str(tr.get("target") or ""))
        hedge = _paper_normalize_venue(str(tr.get("hedge_venue") or ""))
        symbol = str(tr.get("symbol") or "UNKNOWN")
        side = str(tr.get("target_side") or "NONE").upper()
        if not target or not hedge or side not in {"LONG", "SHORT"}:
            continue
        key = f"{strategy}:{target}:{symbol}:{side}:{hedge}"
        groups.setdefault(key, []).append(tr)

    out: List[dict] = []
    for key, rows in groups.items():
        sample = rows[-1]
        strategy = str(sample.get("strategy", "UNKNOWN")).upper()
        nets = [fnum(r.get("realized_net_pnl_pct_x")) for r in rows]
        wins = sum(1 for x in nets if x > 1e-12)
        actual_conv = [max(0.0, min(1.0, fnum(r.get("max_convergence_fraction")))) for r in rows]
        predicted_conv = [
        ...
        predicted_net = [
            fnum(r.get("entry_convergence_adjusted_net_pct"))
            if strategy == "LAG" else fnum(r.get("entry_expected_net_edge_pct"))
            for r in rows
            if (r.get("entry_convergence_adjusted_net_pct") is not None if strategy == "LAG" else r.get("entry_expected_net_edge_pct") is not None)
        ]
        holds = [fnum(r.get("holding_seconds")) for r in rows if r.get("holding_seconds") is not None]
        target = _paper_normalize_venue(str(sample.get("target") or ""))
        hedge = _paper_normalize_venue(str(sample.get("hedge_venue") or ""))
        side = str(sample.get("target_side") or "NONE").upper()
        out.append({
            "key": key,
            "strategy": strategy,
            "target": target,
            "target_label": TARGET_LABELS.get(target, target.title()),
    ...
    out.sort(key=lambda r: (-int(r.get("trades", 0)), -fnum(r.get("median_realized_net_pct_x")), str(r.get("key"))))
    return out[: max(1, int(top_n))]


def build_paper_performance_snapshot(state: dict, cfg: Config, now_ts: Optional[float] = None) -> Optional[dict]:
    """Build a deterministic retrospective snapshot from persisted paper trades.

    The report is based only on information already stored by the paper ledger.
    It never re-selects a hedge venue or changes the historical entry route.
    """
    if not cfg.paper_report_enabled:
        return None
    now_ts = time.time() if now_ts is None else float(now_ts)
    closed = state.get("paper_trades_closed", []) if isinstance(state, dict) else []
    closed = [r for r in closed if isinstance(r, dict) and str(r.get("status", "CLOSED")) == "CLOSED"]
    if len(closed) < max(1, cfg.paper_report_min_closed_trades):
        return None
    open_rows = state.get("paper_trades_open", {}) if isinstance(state, dict) else {}
    skipped = state.get("paper_signals_skipped", []) if isinstance(state, dict) else []
    nets = [fnum(r.get("realized_net_pnl_pct_x")) for r in closed]
    wins = sum(1 for x in nets if x > 1e-12)
    losses = sum(1 for x in nets if x < -1e-12)
    busy_skips = sum(1 for r in skipped if isinstance(r, dict) and str(r.get("reason")) == "VENUE_CAPITAL_BUSY")
    realized_pnl_x = sum(fnum(r.get("realized_pnl_x")) for r in closed)
    source_rows = [
        {
            "id": str(r.get("id") or ""),
            "closed_ts": round(fnum(r.get("closed_ts")), 3),
            "net": round(fnum(r.get("realized_net_pnl_pct_x")), 9),
            "conv": round(fnum(r.get("max_convergence_fraction")), 9),
        }
        for r in closed
    ]
    source_rows.sort(key=lambda r: (r["id"], r["closed_ts"]))
    open_ids = sorted(str(k) for k in open_rows.keys()) if isinstance(open_rows, dict) else []
    fp_payload = {
        "closed": source_rows,
        "open": open_ids,
    ...
    return {
        ...
        "portfolio": {
            ...
            "open_trades": len(open_rows) if isinstance(open_rows, dict) else 0,
            "profitable": wins,
            "losses": losses,
            ...
            "median_realized_net_pct_x": _report_median(nets),
            "mean_realized_net_pct_x": sum(nets) / len(nets) if nets else None,
            "realized_pnl_x": realized_pnl_x,
            "skipped_signals": len(skipped) if isinstance(skipped, list) else 0,
            "venue_capital_busy_skips": busy_skips,
        },
        "lag": _paper_report_strategy_summary(closed, "LAG"),
        "mm": _paper_report_strategy_summary(closed, "MM"),
        "routes": _paper_report_route_rows(closed, cfg.paper_report_top_routes),
    }


def update_paper_performance_report(state: dict, cfg: Config, now_ts: Optional[float] = None) -> bool:
    """Persist a monotonically versioned paper-performance snapshot."""
    snapshot = build_paper_performance_snapshot(state, cfg, now_ts=now_ts)
    if snapshot is None:
        return False
    root = state.setdefault("paper_performance_report", {})
    if not isinstance(root, dict):
        state["paper_performance_report"] = root = {}
    fingerprint = str(snapshot.get("source_fingerprint") or "")
    if fingerprint and fingerprint == str(root.get("source_fingerprint") or ""):
        return False
    version = int(fnum(root.get("current_version"))) + 1
    snapshot["report_version"] = version
    root["schema_version"] = PAPER_REPORT_SCHEMA_VERSION
    root["current_version"] = version
    root["source_fingerprint"] = fingerprint
    root["generated_ts"] = snapshot.get("generated_ts")
    root["current"] = snapshot
    history = root.setdefault("history", [])
    if not isinstance(history, list):
        root["history"] = history = []
    history.append(json.loads(json.dumps(snapshot)))
    cap = max(1, cfg.paper_report_history_limit)
    if len(history) > cap:
        del history[:-cap]
    return True


def format_paper_performance_report(snapshot: dict, cfg: Config) -> str:
    if not isinstance(snapshot, dict):
        return ""
    version = int(fnum(snapshot.get("report_version")))
    p = snapshot.get("portfolio", {}) if isinstance(snapshot.get("portfolio"), dict) else {}
    lag = snapshot.get("lag", {}) if isinstance(snapshot.get("lag"), dict) else {}
    mm = snapshot.get("mm", {}) if isinstance(snapshot.get("mm"), dict) else {}
    def pct(v: Any, digits: int = 3) -> str:
        return "n/a" if v is None else f"{fnum(v):+.{digits}f}%"
    def rate(v: Any) -> str:
        return "n/a" if v is None else f"{fnum(v)*100:.0f}%"
    def sec(v: Any) -> str:
        return "n/a" if v is None else f"{fnum(v):.0f}s"
    lines = [
        f"📊 <b>PAPER PERFORMANCE REPORT #{version}</b>",
        f"Scanner <b>V{html.escape(str(snapshot.get('scanner_version', SCANNER_VERSION)))}</b> | report schema {int(fnum(snapshot.get('schema_version'), PAPER_REPORT_SCHEMA_VERSION))}",
        f"Closed: <b>{int(fnum(p.get('closed_trades')))}</b> | open: {int(fnum(p.get('open_trades')))} | wins/losses: {int(fnum(p.get('profitable')))}/{int(fnum(p.get('losses')))} | win rate <b>{rate(p.get('win_rate'))}</b>",
        f"Realized median: <b>{pct(p.get('median_realized_net_pct_x'))}</b> of X | mean {pct(p.get('mean_realized_net_pct_x'))} | Σ PnL <b>{fnum(p.get('realized_pnl_x')):+.6f} X</b>",
        f"Skipped: {int(fnum(p.get('skipped_signals')))} | venue-capital busy: {int(fnum(p.get('venue_capital_busy_skips')))}",
        "",
        f"<b>LAG</b>: {int(fnum(lag.get('trades')))} trades | win {rate(lag.get('win_rate'))} | predicted conv {rate(lag.get('median_predicted_convergence_fraction'))} | actual max conv {rate(lag.get('median_actual_max_convergence_fraction'))}",
        f"Predicted adjusted net {pct(lag.get('median_predicted_net_pct_x'))} | realized median {pct(lag.get('median_realized_net_pct_x'))} | hold {sec(lag.get('median_hold_seconds'))}",
        f"<b>MM</b>: {int(fnum(mm.get('trades')))} trades | win {rate(mm.get('win_rate'))} | entry net {pct(mm.get('median_predicted_net_pct_x'))} | realized median {pct(mm.get('median_realized_net_pct_x'))} | hold {sec(mm.get('median_hold_seconds'))}",
    ]
    routes = snapshot.get("routes", []) if isinstance(snapshot.get("routes"), list) else []
    if routes:
        lines += ["", "<b>Top exact routes</b>"]
    for i, r in enumerate(routes[: max(1, cfg.paper_report_top_routes)], 1):
        target = html.escape(str(r.get("target_label") or r.get("target") or "?"))
        symbol = html.escape(str(r.get("symbol") or "?"))
        side = html.escape(str(r.get("side") or "?"))
        hedge = html.escape(str(r.get("hedge_venue") or "?").title())
        hedge_side = html.escape(str(r.get("hedge_side") or "?"))
        strategy = html.escape(str(r.get("strategy") or "?"))
        lines.append(f"{i}) <b>{strategy} {target} {symbol} {side} / {hedge} {hedge_side}</b>")
        lines.append(
            f"trades {int(fnum(r.get('trades')))} | profitable {int(fnum(r.get('profitable')))} | win {rate(r.get('win_rate'))} | "
            f"pred conv {rate(r.get('median_predicted_convergence_fraction'))} | actual {rate(r.get('median_actual_max_convergence_fraction'))}"
        )
        lines.append(
            f"pred net {pct(r.get('median_predicted_net_pct_x'))} | realized {pct(r.get('median_realized_net_pct_x'))} | hold {sec(r.get('median_hold_seconds'))}"
        )
    return "\n".join(lines)


def maybe_send_paper_performance_report(state: dict, cfg: Config, dry_run: bool = False, now_ts: Optional[float] = None) -> bool:
    if not cfg.paper_report_enabled or not cfg.paper_report_send_telegram:
        return False
    root = state.get("paper_performance_report", {}) if isinstance(state, dict) else {}
    current = root.get("current", {}) if isinstance(root, dict) else {}
    if not isinstance(current, dict) or not current:
        return False
    version = int(fnum(current.get("report_version")))
    if version <= int(fnum(root.get("last_sent_version"))):
        return False
    now_ts = time.time() if now_ts is None else float(now_ts)
    last_sent_ts = fnum(root.get("last_sent_ts"))
    interval = max(0, cfg.paper_report_interval_minutes) * 60
    if last_sent_ts > 0 and interval > 0 and now_ts - last_sent_ts < interval:
        return False
    text = format_paper_performance_report(current, cfg)
    if not text:
        return False
    if dry_run:
    ...
    if not send_telegram(text):
        return False
    root["last_sent_version"] = version
    root["last_sent_ts"] = int(now_ts)
    return True


def print_paper_portfolio_stats(state: dict) -> None:
    stats = state.get("paper_portfolio_stats", {}) if isinstance(state, dict) else {}
    open_rows = state.get("paper_trades_open", {}) if isinstance(state, dict) else {}
    closed_rows = state.get("paper_trades_closed", []) if isinstance(state, dict) else []
    if not isinstance(stats, dict):
        stats = {}
    opened = int(fnum(stats.get("trades_opened")))
    closed = int(fnum(stats.get("trades_closed")))
    wins = int(fnum(stats.get("wins")))
    losses = int(fnum(stats.get("losses")))
    skipped = int(fnum(stats.get("skipped")))
    net_pct_x = fnum(stats.get("realized_net_pnl_pct_x"))
    pnl_x = fnum(stats.get("realized_pnl_x"))
    open_count = len(open_rows) if isinstance(open_rows, dict) else 0
    trail = len(closed_rows) if isinstance(closed_rows, list) else 0
    win_rate = wins / closed * 100.0 if closed > 0 else 0.0
    print(
        f"Paper portfolio: opened={opened} open={open_count} closed={closed} "
        f"wins/losses={wins}/{losses} win={win_rate:.1f}% skipped={skipped} | "
        f"realized={net_pct_x:+.4f}% of X summed, PnL={pnl_x:+.6f} X | audit rows={trail}"
    )
...
# ---------------------------------------------------------------------------
# V15 economics-aware discovery + per-run gate audit
# ---------------------------------------------------------------------------

def _v14_volume_rank(qvol: float) -> float:
    """Small bounded liquidity tiebreaker; never a LAG eligibility gate."""
    return min(1.0, max(0.0, math.log10(max(qvol, 1.0)) / 10.0))


def prefilter_v10_candidates(snap: V10Snapshot, aster_stats: Dict[str, dict], cfg: Config) -> List[Candidate]:
    """V14 broad discovery.

    LAG path: no 24h-move requirement and no midpoint-deviation requirement.
    We only need a valid fair reference plus executable external BBOs; current
    edge and liquidity are ranking inputs, not eligibility gates.

    MM path: keeps a separate move + volume discovery gate because real trade
    excursions are more productive on active markets.
    """
    all_candidates: List[Candidate] = []
    lag_share = max(0.50, min(0.90, float(cfg.lag_discovery_share)))

    for target in cfg_target_venues():
        books = snap.target_books.get(target, {})
        stats_map = aster_stats if target == "aster" else snap.target_stats.get(target, {})
        lag_rows: List[Tuple[float, Candidate]] = []
        mm_rows: List[Tuple[float, Candidate]] = []
        all_rows: List[Tuple[float, Candidate]] = []
        ...
        for symbol, aq in books.items():
            ...
            hedge_edge = sample.best_hedgeable_edge_pct
            finite_exec = (
                sample.external_bid > 0 and sample.external_ask > 0
                and math.isfinite(sample.hedge_short_edge_pct)
                and math.isfinite(sample.hedge_long_edge_pct)
            )

            # LAG eligibility is deliberately independent of 24h price movement.
            # A positive edge is NOT required here: high-volume seed markets are
            # observed for the whole confirmation window so a gap can appear later.
            lag_eligible = finite_exec
            lag_rank = (
                6.0 * max(0.0, hedge_edge) / max(cfg.min_hedgeable_edge_pct, 1e-9)
                + 1.5 * _v14_volume_rank(qvol)
                + 0.5 * min(1.0, len(getattr(sample, "exec_refs", {})) / max(cfg_min_exec_refs(cfg), 1))
            )

            # MM discovery remains separate from LAG. We seed active/liquid names
            # even if there is no excursion in the one initial snapshot.
            mm_eligible = (
                abs(move) >= cfg.mm_discovery_min_24h_move_pct
                and qvol >= cfg.mm_discovery_min_quote_volume24h
            )
            mm_rank = (
                2.0 * spread / max(cfg.min_aster_spread_pct, 1e-9)
                + 2.0 * dev / max(cfg.min_deviation_pct, 1e-9)
                + min(abs(move) / max(cfg.mm_discovery_min_24h_move_pct, 1e-9), 4.0)
                + _v14_volume_rank(qvol)
            )

            c = _new_candidate(target, symbol, stats, sample, max(lag_rank, mm_rank if mm_eligible else 0.0))
            c.discovery_paths = []
            if lag_eligible:
                c.discovery_paths.append("LAG")
                lag_rows.append((lag_rank, c))
            if mm_eligible:
                c.discovery_paths.append("MM")
                mm_rows.append((mm_rank, c))
            if c.discovery_paths:
                all_rows.append((max(lag_rank if lag_eligible else -999.0, mm_rank if mm_eligible else -999.0), c))

        lag_rows.sort(key=lambda x: x[0], reverse=True)
        mm_rows.sort(key=lambda x: x[0], reverse=True)
        all_rows.sort(key=lambda x: x[0], reverse=True)
        cap = max(1, cfg_target_cap(target, cfg))
        lag_cap = max(1, int(round(cap * lag_share)))
        mm_cap = max(0, cap - lag_cap)

        selected: List[Candidate] = []
        seen: set[str] = set()

        # LAG slots are split between edge-first markets and a deterministic
        # five-minute rotation. This prevents the same high-volume names from
        # monopolising every run while still prioritising a spread that already
        # exists in the initial snapshot.
        edge_share = max(0.25, min(1.0, float(cfg.lag_discovery_edge_share)))
        edge_n = min(lag_cap, max(1, int(round(lag_cap * edge_share))))
        for _score, c in lag_rows[:edge_n]:
            if c.symbol not in seen:
                selected.append(c); seen.add(c.symbol)

        rotate_n = max(0, lag_cap - len(selected))
        if rotate_n > 0:
            pool = [c for _score, c in sorted(lag_rows, key=lambda x: x[1].symbol) if c.symbol not in seen]
            if pool:
                if cfg.lag_discovery_rotation_enabled:
                    slot = int(time.time() // 300)
                    start = (slot * rotate_n) % len(pool)
                    rotated = pool[start:] + pool[:start]
                else:
                    rotated = pool
                for c in rotated[:rotate_n]:
                    if c.symbol not in seen:
                        selected.append(c); seen.add(c.symbol)

        mm_added = 0
        for _score, c in mm_rows:
            if c.symbol in seen:
                existing = next((x for x in selected if x.symbol == c.symbol), None)
                if existing is not None and "MM" not in getattr(existing, "discovery_paths", []):
                    existing.discovery_paths.append("MM")
                continue
            if mm_added >= mm_cap:
                break
            selected.append(c); seen.add(c.symbol); mm_added += 1
        for _score, c in all_rows:
            if len(selected) >= cap:
                break
            if c.symbol in seen:
                continue
        ...
        all_candidates.extend(selected[:cap])

    # Preserve Lighter live trade/book subscriptions for every selected market.
    for c in all_candidates:
        if getattr(c, "target", "aster") == "lighter":
            market_id = int(fnum(getattr(c, "native_id", -1), default=-1))
            _LIGHTER_STREAM.subscribe_trades(market_id)
...
def _v14_lag_failures(m: dict, cfg: Config) -> List[str]:
    fails: List[str] = []
    ref = float(m.get("ref_good_ratio", 0.0))
    side = str(m.get("persistent_edge_side", "NONE"))
    spr = float(m.get("median_spread_pct", math.inf))
    hit = float(m.get("persistent_exec_hit_ratio", 0.0))
    cur = float(m.get("current_executable_edge_pct", 0.0))
    med = float(m.get("persistent_median_executable_edge_pct", 0.0))
    net = float(m.get("current_net_edge_pct", -999.0))
    mednet = float(m.get("persistent_median_net_edge_pct", -999.0))
    if ref < cfg.min_ref_good_ratio: fails.append(f"ref {ref*100:.0f}<{cfg.min_ref_good_ratio*100:.0f}")
    if side not in {"LONG","SHORT"}: fails.append("side none")
    if hit < cfg.min_hedgeable_edge_hit_ratio: fails.append(f"hit {hit*100:.0f}<{cfg.min_hedgeable_edge_hit_ratio*100:.0f}")
    if cur < cfg.min_current_hedgeable_edge_pct: fails.append(f"cur {cur:.3f}<{cfg.min_current_hedgeable_edge_pct:.3f}")
    if med < cfg.min_hedgeable_edge_pct: fails.append(f"med {med:.3f}<{cfg.min_hedgeable_edge_pct:.3f}")
    return fails


def _v14_convergence_failures(m: dict, cfg: Config) -> List[str]:
    if not m.get("raw_lag_confirmed"):
        return []
    status = str(m.get("lag_verification_status", "pending"))
    if status == "not_selected":
        return ["ext not-selected"]
    if status != "done":
        return [f"ext {status}"]
    if m.get("lag_convergence_confirmed"):
        return []
    fails: List[str] = []
    obs = float(m.get("lag_extended_observed_seconds", 0.0))
    conv = float(m.get("lag_max_convergence_fraction", 0.0))
    events = int(m.get("lag_convergence_events", 0))
    cov = float(m.get("lag_bbo_coverage_ratio", 0.0))
    points = int(m.get("lag_valid_bbo_points", 0))
    if obs < cfg.lag_min_extended_observed_seconds: fails.append(f"obs {obs:.0f}s<{cfg.lag_min_extended_observed_seconds:.0f}s")
    if conv < cfg.lag_min_convergence_fraction: fails.append(f"conv {conv*100:.0f}<{cfg.lag_min_convergence_fraction*100:.0f}")
    if events < cfg.lag_min_convergence_events: fails.append(f"events {events}<{cfg.lag_min_convergence_events}")
    if cov < cfg.lag_min_valid_bbo_coverage: fails.append(f"cov {cov*100:.0f}<{cfg.lag_min_valid_bbo_coverage*100:.0f}")
    if points < cfg.lag_min_valid_bbo_points: fails.append(f"pts {points}<{cfg.lag_min_valid_bbo_points}")
    if bool(m.get("lag_baseline_ready")) and not bool(m.get("lag_baseline_anomalous")):
        req = m.get("lag_baseline_required_gap_pct")
        initial = float(m.get("lag_initial_gap_pct", 0.0))
        if req is not None: fails.append(f"base {initial:.3f}<{float(req):.3f}")
        else: fails.append("baseline")
    return fails


def _v14_mm_failures(m: dict, cfg: Config) -> List[str]:
    fails: List[str] = []
    ref = float(m.get("ref_good_ratio", 0.0))
    spr = float(m.get("median_spread_pct", 0.0))
    trades = int(m.get("trades_analyzed", 0))
    exc = int(m.get("excursion_count", 0))
    clean = float(m.get("clean_reversion_rate", 0.0))
    peak = float(m.get("median_peak_excursion_pct", 0.0))
    rev = m.get("median_clean_reversion_seconds")
    if ref < cfg.min_ref_good_ratio: fails.append(f"ref {ref*100:.0f}<{cfg.min_ref_good_ratio*100:.0f}")
    if trades < cfg.min_aster_trades_for_mm: fails.append(f"tr {trades}<{cfg.min_aster_trades_for_mm}")
    if exc < cfg.min_excursions: fails.append(f"exc {exc}<{cfg.min_excursions}")
    if exc > 0:
        if clean < cfg.min_clean_reversion_rate: fails.append(f"clean {clean*100:.0f}<{cfg.min_clean_reversion_rate*100:.0f}")
        if peak < cfg.excursion_threshold_pct: fails.append(f"peak {peak:.3f}<{cfg.excursion_threshold_pct:.3f}")
        if rev is None: fails.append("rev n/a")
        elif float(rev) > cfg.max_median_reversion_seconds: fails.append(f"rev {float(rev):.1f}s>{cfg.max_median_reversion_seconds:.1f}s")
    return fails


def format_scan_summary_chunks(ranked: List[Tuple[Candidate, dict]], cfg: Config) -> List[str]:
    """Compact Telegram audit, one line per selected target:symbol."""
    if not cfg.scan_summary_enabled:
        return []
    lines: List[str] = []
    for c, m in sorted(ranked, key=lambda cm: (str(getattr(cm[0], "target", "aster")), cm[0].symbol)):
        side = str(m.get("current_edge_side", "NONE"))
        gross = float(m.get("current_executable_edge_pct", 0.0))
        lag_f = _v14_lag_failures(m, cfg)
        conv_f = _v14_convergence_failures(m, cfg)
        mm_f = _v14_mm_failures(m, cfg)
        if not lag_f:
            if m.get("lag_convergence_confirmed"):
                lag_txt = "LAG OK+CONV"
            elif conv_f:
                lag_txt = "LAG RAW OK; " + ", ".join(conv_f)
            else:
                lag_txt = "LAG RAW OK"
        else:
            lag_txt = "LAG no: " + ", ".join(lag_f)
        mm_txt = "MM OK" if not mm_f else "MM no: " + ", ".join(mm_f)
        paths = "/".join(getattr(c, "discovery_paths", []) or ["?"])
        raw_line = (
            f"{target_label(c)} {c.symbol} [{paths}] | gross gap {gross:.3f}% {side} | {lag_txt} | {mm_txt}"
        )
        # Telegram messages use HTML parse mode. Gate diagnostics contain raw
        # comparison operators (for example ``hit 20<60``), so escape every
        # dynamic market line before sending it. The header below intentionally
        # keeps its own <b> markup.
        lines.append(html.escape(raw_line, quote=False))

    header = (
        f"📋 <b>V16 SCAN SUMMARY</b> — checked {len(lines)} markets\n"
        "Research gates show gross executable spread only. Trading signals are sent separately only after fresh venue-fee + VWAP + slippage economics are positive.\n"
    )
    max_chars = max(1000, min(3900, int(cfg.scan_summary_max_chars)))
    chunks: List[str] = []
    current = header
    part = 1
    for line in lines:
        piece = line + "\n"
        if len(current) + len(piece) > max_chars and current != header:
            chunks.append(current.rstrip())
            part += 1
            current = f"📋 <b>V16 SCAN SUMMARY continued {part}</b>\n"
        current += piece
    if current.strip():
        chunks.append(current.rstrip())
    return chunks
...
def self_test() -> None:
    cfg = Config(
        ...
        # Keep the built-in self-test deterministic/offline. Execution-depth
        # probing is covered by the dedicated unit tests and must never make
        # live exchange requests from --self-test.
        execution_readiness_enabled=False,
    ...
    sr_events, sr_changed = _v11_detect_same_run_active_lag_events_for_v12([conv], cfg, sr_state)
    ...
    active_events, active_changed = _v11_detect_active_lag_events_for_v12([lag], cfg, active_state)
    ...
    active_events_again, _ = _v11_detect_active_lag_events_for_v12([lag], cfg, active_state)
    ...
    # V14 funding sign convention: positive funding pays SHORT and charges LONG.
    assert abs(_side_funding_pnl_pct("SHORT", [0.01, -0.02]) - (-0.01)) < 1e-12
    assert abs(_side_funding_pnl_pct("LONG", [0.01, -0.02]) - (0.01)) < 1e-12

    # V14 strict horizon cutoff: a quote arriving AFTER the configured outcome
    ...
    # V16 persistent paper ledger: actionable convergence-aware signal stores
    # the exact target/hedge route and normalized entry VWAPs. Realized PnL is
    # measured from executable close VWAPs on the same route, not from a new
    # best venue selected with hindsight.
    paper_cfg = replace(cfg, paper_trading_enabled=True, paper_enforce_venue_capital=False,
                        paper_min_entry_net_edge_pct=0.0, funding_tracking_enabled=False)
    paper_state = {}
    paper_event = {
        "type": "ACTIVE-LAG", "symbol": "PAPERUSDT", "state_key": "aster:PAPERUSDT",
        "target": "aster", "target_label": "Aster", "ts": now, "side": "SHORT",
        "execution_ready": True, "execution_min_notional_usd": 100,
        "execution_best_external_venue": "lighter-perp",
        "execution_fresh_gross_edge_pct": 0.30, "execution_fresh_net_edge_pct": 0.22,
        "execution_checks": {"100": {
            "status": "READY", "gross_edge_pct": 0.30, "net_edge_pct": 0.22,
            "external_venue": "lighter-perp", "target_vwap": 100.30, "external_vwap": 100.00,
            "target_taker_fee_pct": 0.04, "external_taker_fee_pct": 0.00,
            "exit_slippage_reserve_pct": 0.0,
        }},
        "verified_episodes": 2, "median_max_convergence_fraction": 0.80,
    }
    assert open_paper_trade_from_signal(paper_state, paper_event, paper_cfg)
    ptrade = next(iter(paper_state["paper_trades_open"].values()))
    assert ptrade["hedge_venue"] == "lighter-perp" and ptrade["entry_target_vwap"] == 100.30
    ptarget = {"bids": [(100.05, 1000.0)], "asks": [(100.06, 1000.0)], "venue": "aster"}
    phedge = {"bids": [(99.99, 1000.0)], "asks": [(100.00, 1000.0)], "venue": "lighter-perp"}
    pmark = _paper_mark_from_books(ptrade, ptarget, phedge, paper_cfg, now + 300)
    assert pmark is not None and pmark["net_pnl_pct_x"] > 0, pmark
    assert _paper_close_reason(ptrade, pmark, paper_cfg, now + 300) in {"LAG_FULL_CONVERGENCE", "LAG_CONVERGENCE"}

    print(
        "Self-test OK (V16.4 route-specific convergence economics + baseline gate + persistent paper ledger)"
    )
...
def main() -> int:
    ...
    print("Config:", cfg)
    state = load_state()
    callback_state_changed = False

    def active_signal_callback(event: dict) -> None:
        nonlocal callback_state_changed
        etype = str(event.get("type", ""))
        # V16 paper entries are created only from convergence/reversion-aware
        # actionable signals, never from broad spread-only economics alerts.
        if etype in {"ACTIVE-LAG", "ACTIVE-MM-EXCURSION"}:
            callback_state_changed = open_paper_trade_from_signal(state, event, cfg) or callback_state_changed
    ...
    # V14: every run emits a compact market-by-market gate audit. This is text
    # only; normal CONFIRMED/ACTIVE signal delivery remains unchanged.
    summary_chunks = format_scan_summary_chunks(ranked, cfg)
    ...
    # V16: mark every persisted paper trade on the exact stored route. New
    # trades created during this run are also eligible for an immediate close if
    # their convergence/reversion completed before the run ended.
    paper_mark_changed = update_paper_trades(state, cfg)
    report_changed = update_paper_performance_report(state, cfg)
    report_delivery_changed = maybe_send_paper_performance_report(state, cfg, dry_run=args.dry_run)
    print_paper_portfolio_stats(state)
    ...
    state_changed = bool(scan_state_changed or callback_state_changed or paper_mark_changed or report_changed or report_delivery_changed)
    ...
    for candidate, m in ranked:
        ...
        # V15 invariant: no user-facing CONFIRMED/MM/LAG entry-style alert is
        # sent unless a fresh $minimum-notional hedged route has positive
        # conservative economics after venue fees, VWAP and exit-slip reserve.
        preferred_side = str(m.get("persistent_edge_side", "NONE"))
        econ_side, econ_probe = best_positive_execution_probe(
            candidate, m, cfg, preferred_side=preferred_side, force_fresh=True
        )
        if econ_probe is None or not econ_probe.get("execution_ready"):
            continue
        alert_m = dict(m)
        alert_m["_economic_probe"] = econ_probe
        alert_m["_economic_side"] = econ_side

        text = format_alert(candidate, alert_m)
        if args.dry_run:
            print("\nDRY ALERT:\n", text.replace("<b>", "").replace("</b>", ""))
            continue
        ...
        if send_telegram(text):
            if level in cfg.chart_alert_levels:
                chart_path = create_price_chart(candidate, alert_m, cfg)
                if chart_path is not None:
                    if send_telegram_photo(chart_path, candidate, alert_m):
